...
// process via the CRT invalid parameter handler. Callers must ensure fd is
// valid.
...
MMAPAllocation::MMAPAllocation(int fd, ErrorReporter* error_reporter,
                               bool map_private)
    // On Windows, duplicating an invalid fd (e.g. -1) crashes the process.
    // Guard against it.
    : MMAPAllocation(error_reporter, fd >= 0 ? DuplicateFd(fd) : -1,
                     map_private) {
  if (mmap_fd_ == -1) {
...
MMAPAllocation::MMAPAllocation(int fd, size_t offset, size_t length,
    ...
    // On Windows, duplicating an invalid fd (e.g. -1) crashes the process.
    // Guard against it.
    : MMAPAllocation(error_reporter, fd >= 0 ? DuplicateFd(fd) : -1, offset,
                     length, map_private) {
  if (mmap_fd_ == -1) {
    TF_LITE_REPORT_ERROR(error_reporter, "Failed to dup '%d' file descriptor.",
                         fd);
...
#if defined(_WIN32)
  fprintf(stderr, "MMAPAllocation: Mapped file on Windows.\n");
#else
  fprintf(stderr, "MMAPAllocation: Mapped file using mmap.\n");
#endif
}
