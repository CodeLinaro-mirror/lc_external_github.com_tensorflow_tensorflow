...
absl::StatusOr<std::shared_ptr<xla::PjRtBuffer>> MakeBufferUndonatable(
  ...
  if (common_pjrt_buffer == nullptr) {
    LOG(WARNING)
        << "Undonatable variable buffers requested, but the PjRt buffer is not "
           "backed by CommonPjRtBuffer; keeping donatable buffers.";
    return pjrt_buffer;
  }
  ...
  // Copy the raw buffer and definition events before ConfirmDonation().
  // ConfirmDonation() releases the tracked buffer's reference to the device
  // memory.
