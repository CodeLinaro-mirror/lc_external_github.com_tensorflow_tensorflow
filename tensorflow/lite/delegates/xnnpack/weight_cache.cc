...
bool MMapWeightCacheProvider::SyncCacheMissesToFile() {
...
MMapWeightCacheProvider::~MMapWeightCacheProvider() { SyncCacheMissesToFile(); }
...
bool MMapWeightCacheProvider::Load() {
  ...
  if (const auto buffers = buffer_list->buffers(); buffers) {
    for (auto* buffer : *buffers) {
      ...
      max_buffer_offset = std::max(max_buffer_offset, buffer->offset());
    }
  }
