...
struct Tensor {
  ...
  int64_t size() const {
    ...
    ABSL_DCHECK(!total_size.Overflow());
  ...
  std::vector<int64_t> Strides() const {
    ABSL_DCHECK_LE(shape.size(), kMaxDims);
...
inline std::vector<int64_t> ExtendToMaxDims(std::vector<int64_t> vec,
                                            int64_t val) {
  ABSL_DCHECK_LE(vec.size(), kMaxDims);
...
inline std::vector<int64_t> DilateShape(std::vector<int64_t> shape,
  ...
  ABSL_DCHECK_GE(dilations.size(), shape.size());
...
inline std::vector<int64_t> PadCropShape(std::vector<int64_t> shape,
  ...
  ABSL_DCHECK_GE(padding.size(), 2 * shape.size());
  for (size_t i = 0; i < shape.size(); ++i) {
...
Tensor<T> Pad(const Tensor<T>& input, const std::vector<int64_t>& padding,
              const T padding_value) {
  ABSL_DCHECK_LE(padding.size(), kMaxDims * 2);
  ABSL_DCHECK_GE(padding.size(), 2 * input.shape.size());
...
Tensor<T> Crop(const Tensor<T>& input, const std::vector<int64_t>& cropping) {
  ABSL_DCHECK_LE(cropping.size(), kMaxDims * 2);
  ABSL_DCHECK_GE(cropping.size(), 2 * input.shape.size());
...
inline std::vector<int64_t> ReduceWindowShape(
  ...
  ABSL_DCHECK_GE(base_dilations.size(), shape.size());
  ABSL_DCHECK_GE(padding.size(), 2 * shape.size());
  ABSL_DCHECK_GE(window_dimensions.size(), shape.size());
  ABSL_DCHECK_GE(window_dilations.size(), shape.size());
  ABSL_DCHECK_GE(window_strides.size(), shape.size());
  ...
  for (size_t i = 0; i < base_shape.size(); ++i) {
    ABSL_DCHECK_GT(window_strides[i], 0);
