...
// measured stability, correctness, and performance. For more detail see
// README.md.
//
// Example usage:
...
//   EXPECT_OK(tflite::gpu::gl::RequestGpuInfo(&gpu_info));
//   auto list = tflite::acceleration::GPUCompatibilityList::Create();
//   TfLiteDelegate* gpu_delegate = nullptr;
//   TfLiteGpuDelegateOptionsV2 gpu_options;
//   if (list && list->Includes(android_info, gpu_info)) {
//     gpu_options = list->GetBestOptionsFor(android_info, gpu_info);
...
class GPUCompatibilityList {
 public:
  // Construct list from bundled data. Returns nullptr if creation fails.
  static std::unique_ptr<GPUCompatibilityList> Create();

  // Constructs list from the given flatbuffer data. Returns nullptr if the
  // given flatbuffer is empty or invalid.
  ...
  static std::map<std::string, std::string> InfosToMap(
  ...
  // string.
  static absl::string_view CompatibilityStatusToString(
 ...
 private:
  const DeviceDatabase* database_ = nullptr;
