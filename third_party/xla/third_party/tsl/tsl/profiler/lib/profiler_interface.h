...
class ProfilerInterface {
 public:
  ...
  // Consumes data from the profiler incrementally.
  virtual absl::Status Consume(std::string* output) {
    return absl::UnimplementedError("Consume is not implemented.");
  }

  // Serializes consumed data into the provided output buffer.
  virtual absl::Status Serialize(const std::string& input,
                                 std::string* output) {
