...
class TiledHloComputation {
 public:
  ...
  const TiledHloRegion& tiled_root_region() const { return tiled_root_region_; }
  ...
  instructions() const {
    return {MakeUnwrappingIterator(tiled_root_region_.instructions().begin()),
            MakeUnwrappingIterator(tiled_root_region_.instructions().end())};
  }
  ...
  absl::Span<const TiledHloInstruction* const> roots() const {
    return tiled_root_region_.roots();
  }
 ...
 private:
  TiledHloComputation(
      ...
      : tiling_space_(std::move(tiling_space)),
        tiled_root_region_(std::move(tiled_root_region)),
  ...
  TiledHloRegion tiled_root_region_;
