...
//
// -----------------------------------------------------------------------------
// PATTERN 2: STEPPED-STRIDE (NON-CONTIGUOUS) TILING (Phase 2)
// -----------------------------------------------------------------------------
// We allow EXACTLY ONE dimension on the multidim side to have a stride > 1.
// This is the "stepped" dimension.
//
// --- Collapse [3, 4] -> [12] ---
...
//       Matches flat stride of target Dim 0. Target Dim 0 is the stepped dim.
...
absl::Status VerifyReshapeContiguity(
  ...
  // ===========================================================================
  // 1. Stride Verification & Stepped Dimension Identification
  // ===========================================================================
  ...
  // Identify which dimensions (if any) are stepped (stride > 1).
  ...
  //   stepped. For target shape [D0, D1, D2], flat strides are [D1*D2, D2, 1].
  std::vector<int> stepped_dims;
  if (is_collapse) {
    for (int j = 0; j < source_strides.size(); ++j) {
      if (source_strides[j] > 1) {
        stepped_dims.push_back(j);
      }
    }
    ...
    for (int i = n - 2; i >= 0; --i) {
      ...
      if (source_strides[0] == flat_stride) {
        stepped_dims.push_back(i);
        break;
      }
    }
    if (stepped_dims.empty()) {
      return FormatError(
  ...
  // We only support at most one stepped dimension.
  if (stepped_dims.size() > 1) {
    return FormatError(
        "At most one dimension can have stride >1, found stepped dimensions: [",
        absl::StrJoin(stepped_dims, ", "), "]");
  }

  // ===========================================================================
  // 2. Non-Contiguous Tiling Verification (Has Stepped Dimension)
  // ===========================================================================
  // If a stepped dimension is present, we enforce a strict rule: No other
  // dimension on the multidim side can be tiled (all others must have size 1).
  if (!stepped_dims.empty()) {
    int stepped_dim = stepped_dims[0];
    for (int j = 0; j < n; ++j) {
      if (j != stepped_dim &&
          !IsConstantValue(multidim_side_tiles[j].size, 1)) {
        return FormatError(
            "For non-contiguous source tile stride, only the stepped "
            "dimension ",
            stepped_dim, " can have size > 1. Found: dim ", j, " has size ",
