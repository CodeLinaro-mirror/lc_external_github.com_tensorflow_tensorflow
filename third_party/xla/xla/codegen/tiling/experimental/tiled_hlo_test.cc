...
TEST_F(TileAnalysisTest, CollectiveDotBasic) {
  ...
  EXPECT_THAT(tiled_computation, MatchString(R"(
    ...
    Tiled HLO:
      ...
      region #0 {
      p0.1.tile_0 = parameter(0)  offsets [(tid_0 * 16) mod 64, tid_2 * 32] sizes [16, 32] strides [1, 1] upper bounds [64, 256]
                                  replica ids { offsets [(tid_0 * 16) floordiv 64] sizes [1] strides [1] upper bounds [2] }
