...
TEST_F(SchedulingTest, GetDotPermutationSmallLhs) {
  ASSERT_OK_AND_ASSIGN(const TiledHloComputation tiled_computation,
                       ParseAndTile(R"(
    fusion {
      p0 = f32[16,128]{1,0} parameter(0)
      p1 = f32[128,4096]{1,0} parameter(1)
      ROOT dot = f32[16,4096]{1,0} dot(p0, p1),
        lhs_contracting_dims={1}, rhs_contracting_dims={0}
    }
    ENTRY main {
      p0 = f32[16,128]{1,0} parameter(0)
      p1 = f32[128,4096]{1,0} parameter(1)
      ROOT fusion = f32[16,4096]{1,0} fusion(p0, p1), kind=kLoop, calls=fusion
    })",
                                    {8, 64, 32}));
  EXPECT_THAT(
      GetSchedule(tiled_computation),
      IsOkAndHolds(MatchSchedule(
          "d0 -> pid mod 2, d1 -> pid floordiv 2, pid_bounds=[0, 127]")));
}

TEST_F(SchedulingTest, GetDotPermutationLargeLhs) {
  ASSERT_OK_AND_ASSIGN(const TiledHloComputation tiled_computation,
                       ParseAndTile(R"(
    fusion {
      p0 = f32[4096,128]{1,0} parameter(0)
      p1 = f32[128,16]{1,0} parameter(1)
      ROOT dot = f32[4096,16]{1,0} dot(p0, p1),
        lhs_contracting_dims={1}, rhs_contracting_dims={0}
    }
    ENTRY main {
      p0 = f32[4096,128]{1,0} parameter(0)
      p1 = f32[128,16]{1,0} parameter(1)
      ROOT fusion = f32[4096,16]{1,0} fusion(p0, p1), kind=kLoop, calls=fusion
    })",
                                    {64, 8, 32}));
  EXPECT_THAT(
      GetSchedule(tiled_computation),
      IsOkAndHolds(MatchSchedule(
          "d0 -> pid floordiv 2, d1 -> pid mod 2, pid_bounds=[0, 127]")));
}
