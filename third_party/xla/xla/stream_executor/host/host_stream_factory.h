...
class HostStreamFactory {
 public:
  ...
  // Returns a shared pointer to the currently registered HostStreamFactory.
  static std::shared_ptr<const HostStreamFactory> GetFactory();
};
...
#define INTERNAL_REGISTER_LOCAL_HOST_STREAM_FACTORY_NAME(ctr) \
  host_stream_factory_registrar_##ctr
