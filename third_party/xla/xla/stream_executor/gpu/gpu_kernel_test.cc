...
class GpuKernelTest : public ::testing::Test {
 public:
  ...
  void RunTmaKernel(const KernelLoaderSpec& spec) {
    if (executor_->GetPlatform()->id() ==
        stream_executor::rocm::kROCmPlatformId) {
      GTEST_SKIP() << "TMA is not supported on ROCm.";
    }

    TF_ASSERT_OK_AND_ASSIGN(auto stream, executor_->CreateStream());
    TF_ASSERT_OK_AND_ASSIGN(auto tma_kernel,
                            TmaKernel::Create(executor_, spec));
    ...
    TF_ASSERT_OK_AND_ASSIGN(
        auto tma0, executor_->CreateTensorMap(arg0_desc, mem0.opaque()));
    TF_ASSERT_OK_AND_ASSIGN(
        auto tma1, executor_->CreateTensorMap(arg1_desc, mem1.opaque()));
    TF_ASSERT_OK_AND_ASSIGN(
        auto tma2, executor_->CreateTensorMap(arg2_desc, mem2.opaque()));
    TF_ASSERT_OK_AND_ASSIGN(
        std::unique_ptr<KernelArgs> packed_args,
        stream_executor::PackKernelArgs(
            absl::Span<const stream_executor::KernelArgument>(
                {tma0, tma1, tma2}),
            tma_kernel->metadata()));
    TF_ASSERT_OK(tma_kernel->Launch(ThreadDim(), BlockDim(), stream.get(),
                                    *packed_args));
...
TEST_F(GpuKernelTest, TmaLoadAndRunKernelFromPtx) {
  ...
  RunTmaKernel(GetTmaPtxKernelSpec());
}
