...
class CudaBlasLtTest : public ::testing::Test {
 protected:
  ...
  void RunS8S8Gemm(blas::ComputationType compute_type) {
    ...
    // S8 Gemms with s8 output are not supported on Tesla V100 (compute
    // capability 7.0).
    if (!cuda_cc->IsAtLeast(6, 1) ||
        (output_primitive_type == xla::PrimitiveType::S8 &&
         !cuda_cc->IsAtLeast(7, 5))) {
    ...
    TF_ASSERT_OK(plan->SetAlgorithm(algorithms[0]));
    ...
    TF_ASSERT_OK(plan->ExecuteOnStream(stream_.get(), args, nullptr));

    std::vector<OutputT> h_result(m * n);
    TF_ASSERT_OK(stream_->Memcpy(h_result.data(), d_c,
...
TEST_F(CudaBlasLtTest, S8S8S8Gemm) {
  RunS8S8Gemm<int8_t>(blas::ComputationType::kI32);
}
