...
absl::StatusOr<int> GetPtxVersion() {
  int device;
  TF_RETURN_IF_ERROR(ToStatus(cudaGetDevice(&device)));
  int major, minor;
  TF_RETURN_IF_ERROR(ToStatus(cudaDeviceGetAttribute(
...
struct CubScanDispatcher {
  DispatchT dispatch;
  template <typename ChainedPolicyT>
  cudaError_t Invoke() const {
    return dispatch(ChainedPolicyT::ActivePolicy::ScanPolicyT::BLOCK_THREADS);
  }
};
...
absl::Status CubScanDispatch(void* d_temp_storage, size_t* temp_bytes,
  ...
  auto dispatch = [&](int block_size) {
    void* kernel_args[] = {(void*)&d_in, (void*)&d_out, (void*)&row_length};
    return cudaLaunchKernel(BlockScanKernel<T, ScanOpT, MaxPolicyT>,
                            column_length, block_size, kernel_args, 0, stream);
  };
  CubScanDispatcher<decltype(dispatch)> dispatcher{dispatch};

  TF_ASSIGN_OR_RETURN(int ptx_version, GetPtxVersion());
  return ToStatus(MaxPolicyT::Invoke(ptx_version, dispatcher));
}
