...
#else
    // Pre-Pascal architectures do not support atomic fetch_add, so the build
    // fails if we attempt to use it. We must check that the GPU is at least
    // Pascal before launching the kernel.
    assert(false);
#endif
  }
}
