...
absl::StatusOr<std::vector<uint8_t>> BundleGpuAsmUsingFatbin(
  ...
  for (int i = 0; i < images.size(); i++) {
    absl::string_view variant = ShouldUsePtxExtension(images[i].cc) ? "a" : "";
    absl::string_view kind = images[i].is_ptx ? "ptx" : "elf";
    fatbinary_args.push_back(absl::StrFormat(
        "--image3=kind=%s,sm=%d%d%s,file=%s", kind, images[i].cc.major,
        images[i].cc.minor, variant, image_paths[i]));
  }
  if (VLOG_IS_ON(3)) {
