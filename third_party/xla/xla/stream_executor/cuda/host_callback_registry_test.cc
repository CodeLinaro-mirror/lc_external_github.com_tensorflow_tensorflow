...
TEST_F(HostCallbackRegistryTest, BackgroundEnqueue) {
  absl::Notification done;
  HostCallbackRegistry registry = CreateRegistry();
...
TEST_F(HostCallbackRegistryTest, BackgroundEnqueueFailure) {
  ...
  args.synchronization_callback = [&, first_call = true]() mutable {
    ...
    if (first_call) {
      background_enqueue.cancelled->store(true, std::memory_order_release);
  ...
  // Delete the thread pool before the registry.
  background_enqueue.enqueue_cb = nullptr;
}
...
TEST_F(HostCallbackRegistryTest, BackgroundEnqueueNoSyncOnFailure) {
  ...
  std::atomic<int32_t> error_count = 0;
  {
    ...
    for (int i = 0; i < 10; ++i) {
      absl::Status status = registry.AddCallback(
          [&]() {
            int32_t current = ++success_count;
            if (current == 5) {  // The 5th callback will not notify error.
              error_notified.Notify();
            }
            absl::SleepFor(absl::Milliseconds(1));  // Simulation some work.
            return absl::OkStatus();
          },
  ...
  EXPECT_GE(success_count, 5);  // At least five succeeded. Maybe more.
}
}  // namespace
}  // namespace stream_executor::gpu
