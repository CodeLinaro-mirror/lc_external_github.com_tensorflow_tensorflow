...
absl::StatusOr<CudaDeviceAllocator::Options> QueryDeviceAllocatorOptions(
  ...
  if (!status.ok()) {
    XLA_LOG_DEVICE(WARNING, device)
        << "Device allocator granularity query with HANDLE_TYPE_NONE failed: "
        << status << "; device allocation is not supported for this device.";
    return status;
  }
...
CudaDeviceAllocator::CudaDeviceAllocator(StreamExecutor* executor)
  ...
  if (!status.ok()) {
    XLA_LOG_DEVICE(ERROR, executor_->device_ordinal())
        << "Failed to get CUDA device while initializing device allocator: "
        << status;
    return;
  }

  auto options = QueryDeviceAllocatorOptions(device);
  if (!options.ok()) {
    XLA_LOG_DEVICE(ERROR, executor_->device_ordinal())
        << "Failed to query device allocator options while initializing device "
           "allocator: "
        << options.status();
    return;
  }
