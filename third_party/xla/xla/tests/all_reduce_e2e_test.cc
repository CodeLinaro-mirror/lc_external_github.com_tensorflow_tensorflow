...
struct AllReduceTestParams {
  ...
  // Returns the maximum number of elements that can be supported for the given
  // element type and strategy.
  int64_t NumElements(PrimitiveType element_type) const {
    int64_t max_supported_size =
        gpu::GetMaxSupportedAllReduceSizeBytes(strategy);
...
class AllReduceTest
 ...
 protected:
  ...
  static absl::StatusOr<InputsOutputs> BuildTestInputsOutputs(
    ...
    const int64_t num_elements = GetParam().NumElements(element_type);
    CHECK_EQ(Product(module.entry_computation()
                         ->root_instruction()
                         ->shape()
                         .dimensions()),
             num_elements);
