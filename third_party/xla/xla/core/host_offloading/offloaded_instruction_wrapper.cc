...
#include "xla/core/host_offloading/offloaded_instruction_wrapper.h"
...
FindAndWrapOffloadedComputations(
  ...
  while (true) {
    ...
    for (auto* instr : iter::reversed(computation.MakeInstructionPostOrder())) {
      ...
      if (offloaded_call_instr == nullptr) {
        ...
        offloaded_call_instr = down_cast<HloCallInstruction*>(call_instr);
