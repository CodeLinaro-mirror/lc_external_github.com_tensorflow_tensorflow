...
func.func @main(%arg0: tensor<6xf32>, %arg1: tensor<6xf32>, %arg2: tensor<3xi32>, %arg3: tensor<3xi32>, %arg4: tensor<3xi32>, %arg5: tensor<3xi32>) -> (tensor<6xf32>) {
  %0 = mhlo.custom_call @ragged_all_to_all(%arg0, %arg1, %arg2, %arg3, %arg4, %arg5) {api_version = 4 : i32, backend_config = {channel_id = 1 : i64, replica_groups = dense<[[0, 1, 2]]> : tensor<1x3xi64>}} : (tensor<6xf32>, tensor<6xf32>, tensor<3xi32>, tensor<3xi32>, tensor<3xi32>, tensor<3xi32>) -> tensor<6xf32>
...
func.func @main(%token: !mhlo.token) -> !mhlo.token {
  %0 = "mhlo.send"(%token) {
    ...
    is_host_transfer = false,
    source_target_pairs = dense<[[0,1],[1,2]]> : tensor<2x2xi64>,
    mhlo.frontend_attributes = {_xla_host_transfer_rendezvous = "fe_attr_just_for_testing"}
  } : (!mhlo.token) -> !mhlo.token
  func.return %0 : !mhlo.token
}
...
// CHECK-SAME: frontend_attributes
// CHECK-SAME{LITERAL}: _xla_host_transfer_rendezvous="fe_attr_just_for_testing",
// CHECK-SAME: _xla_send_recv_source_target_pairs
// CHECK-SAME{LITERAL}: {{0,1},{1,2}}
// CHECK-NEXT:  ROOT
// CHECK-SAME:   token[] send-done([[SEND]]), channel_id=5
...
// CHECK: HloModule
// CHECK: ENTRY
// CHECK: f32[192] parameter(0), origin={{[{][{]}}"a"{{[}][}]}}
...
// CHECK: HloModule
// CHECK: ENTRY
// CHECK: ROOT %constant.1 = s32[] constant(0), origin={{[{][{]}}"constant.5"{{[}][}]}}
...
// CHECK: ENTRY
// CHECK:  %[[ARG0:.*]] = f32[10] parameter(0)
// CHECK:  %[[TOPK:.*]] = (f32[8], s32[8]) topk(%[[ARG0]]), k=8, largest=true, origin={({"t" {0}{{[}]}}, {"t" {1}{{[}]}})}
...
// CHECK: HloModule
// CHECK: ENTRY
// CHECK:  %[[CONSTANT:.*]] = s32[] constant(0), origin={{[{][{]}}"c"{{[}][}]}}
// CHECK:  %[[ARG0:.*]] = s32[] parameter(0), origin={{[{][{]}}"a"{{[}][}]}}
// CHECK:  %[[ARG1:.*]] = s32[] parameter(1), origin={{[{][{]}}"b"{{[}][}]}}
// CHECK:   tuple(%[[CONSTANT]], %[[ARG0]], %[[ARG1]]), origin={({"c"}, {"a"}, {"b"})}
