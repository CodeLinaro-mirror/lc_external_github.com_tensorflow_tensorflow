...
std::string ConvKindToString(ConvolutionKind kind) {
  switch (kind) {
...
HloConvolutionInstruction::HloConvolutionInstruction(
    ...
    const PrecisionConfig& precision_config,
    const SparsityConfig& sparsity_config, ConvolutionKind conv_kind)
...
void HloConvolutionInstruction::ToProto(HloInstructionProto* proto) const {
  ...
  *proto->mutable_precision_config() = precision_config_;
  if (conv_kind_ != CONVOLUTION_KIND_UNSET) {
    proto->set_conv_kind(conv_kind_);
  }
...
HloConvolutionInstruction::CloneWithNewOperandsImpl(
  ...
  return std::make_unique<HloConvolutionInstruction>(
      ...
      precision_config_, sparsity_config_, conv_kind_);
}
