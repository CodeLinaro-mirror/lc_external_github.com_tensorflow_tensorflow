...
MeshAxesReplicaGroupList::MeshAxesReplicaGroupList(Mesh mesh,
  ...
  for (const AxisRef& axis : axes_) {
    ...
    if (axis.sub_axis_info().has_value()) {
      CHECK(mesh_.device_assignment().dim(axis.mesh_axis_index()) %
  ...
  for (auto& [dim, axes] : dim_to_axes) {
    int64_t full_axis_size = mesh_.device_assignment().dim(dim);
    ...
    dim_to_reshape_and_aggregate_axes_[dim] = reshape_and_aggregate_axes;
  }
}
...
std::vector<std::vector<int64_t>> get_replica_groups_for_full_axes(
    ...
    const std::vector<int64_t>& grouped_axes, const int64_t num_replica_groups,
  ...
  for (int64_t i = 0; i < axis_sizes.size(); ++i) {
    auto it = absl::c_find(grouped_axes, i);
    if (it == grouped_axes.end()) {
...
MeshAxesReplicaGroupList::flattened_replica_groups() const {
  ...
  for (int64_t i = 0; i < mesh_.device_assignment().dimensions().size(); ++i) {
    int64_t axis_size = mesh_.device_assignment().dim(i);

    auto it = dim_to_reshape_and_aggregate_axes_.find(i);
    if (it == dim_to_reshape_and_aggregate_axes_.end()) {
