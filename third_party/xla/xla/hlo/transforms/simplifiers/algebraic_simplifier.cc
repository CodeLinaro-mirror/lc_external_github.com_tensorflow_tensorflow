...
absl::Status AlgebraicSimplifierVisitor::HandleSlice(HloInstruction* slice) {
  ...
  if (Match(slice, m::Slice(m::Reshape(&reshape, m::Op())))) {
    ...
    if (reshape_rank >= 2 && reshape_rank == slice_rank) {
      ...
      for (int64_t i = 0; i < slice_rank - 1; ++i) {
        if (slice->slice_starts()[i] != 0 ||
      ...
      if (is_valid_reshape_slice &&
          slice->slice_limits()[slice_rank - 1] -
                  slice->slice_starts()[slice_rank - 1] ==
              1 &&
          slice->slice_strides()[slice_rank - 1] == 1) {
        int64_t slice_index = slice->slice_starts()[slice_rank - 1];
        int64_t K = reshape_shape.dimensions(slice_rank - 1);
