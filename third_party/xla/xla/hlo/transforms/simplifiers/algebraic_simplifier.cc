...
absl::Status AlgebraicSimplifierVisitor::HandleConvert(
  ...
  // Simplify convert(broadcast(constant)) to broadcast(convert(constant)).
  // This pushes the convert through the broadcast, which is cheap for scalars,
  // and exposes the broadcast directly to other optimizations (e.g.
  // ReduceWindow).
  if (!options_.is_layout_sensitive() &&
      convert->operand(0)->opcode() == HloOpcode::kBroadcast &&
      convert->operand(0)->operand(0)->IsConstant()) {
    auto* bcast = convert->mutable_operand(0);
    auto* constant = bcast->mutable_operand(0);
    if (ShapeUtil::IsScalar(constant->shape())) {
      TF_ASSIGN_OR_RETURN(Literal dest_literal,
                          constant->literal().Convert(dest_type));
      auto* new_constant = convert->AddInstruction(
          HloInstruction::CreateConstant(std::move(dest_literal)));
      return ReplaceWithNewInstruction(
          convert, HloInstruction::CreateBroadcast(
                       convert->shape(), new_constant, bcast->dimensions()));
    }
  }

  // Simplify convert(reshape(X)) to reshape(convert(X)).
  if (!options_.is_layout_sensitive() &&
      convert->operand(0)->opcode() == HloOpcode::kReshape) {
    auto* reshape = convert->mutable_operand(0);
    auto* x = reshape->mutable_operand(0);
    Shape new_convert_shape =
        ShapeUtil::ChangeElementType(x->shape(), dest_type);
    auto* new_convert = convert->AddInstruction(
        HloInstruction::CreateConvert(new_convert_shape, x));
    return ReplaceWithNewInstruction(
        convert, HloInstruction::CreateReshape(convert->shape(), new_convert));
  }
...
absl::Status AlgebraicSimplifierVisitor::HandleReduceWindow(
  ...
  if (real_operand->opcode() == HloOpcode::kBroadcast &&
      real_operand->operand(0)->IsConstant() &&
