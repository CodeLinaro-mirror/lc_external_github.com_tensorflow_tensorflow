...
struct WeightedSlice {
  ...
  Literal weight;
  HloInstruction* array_weight = nullptr;

  bool is_scalar() const { return array_weight == nullptr; }
};
...
bool CompareWeightedSlices(const WeightedSlice& a, const WeightedSlice& b) {
  ...
  if (!a.array_weight && !b.array_weight) {
    return false;
  } else if (!a.array_weight) {
    return true;
  } else if (!b.array_weight) {
    return false;
  } else {
    return CompareInstruction(a.array_weight, b.array_weight);
  }
}

void ExtractWeightedSlices(
    HloInstruction* inst, std::vector<WeightedSlice>& slices,
    std::optional<Literal> current_weight = std::nullopt) {
  HloEvaluator evaluator;
  Literal cw;
  if (!current_weight.has_value()) {
    cw = LiteralUtil::One(inst->shape().element_type());
  } else {
    cw = std::move(*current_weight);
  }
    ...
    if (val.has_value() && other != nullptr) {
      Literal next_weight =
          evaluator.EvaluateElementwiseBinaryOp(HloOpcode::kMultiply, cw, *val)
              .value();
      ExtractWeightedSlices(other, slices, std::move(next_weight));
      return;
    } else {
      slices.push_back({lhs, std::move(cw), rhs});
      return;
    }
  }
  if (inst->opcode() == HloOpcode::kAdd) {
    ExtractWeightedSlices(inst->mutable_operand(0), slices, cw.Clone());
    ExtractWeightedSlices(inst->mutable_operand(1), slices, std::move(cw));
    return;
  }
      ...
      for (int64_t i = 0; i < num_slices; ++i) {
        ...
        if (reshape->opcode() != HloOpcode::kReshape ||
            (reshape->operand(0)->opcode() != HloOpcode::kSlice &&
             reshape->operand(0)->opcode() != HloOpcode::kPad)) {
          all_good = false;
          break;
        }
        ...
        temp_slices.push_back(
            {reshape->mutable_operand(0), std::move(next_w), nullptr});
      }
      if (all_good) {
  ...
  slices.push_back({inst, std::move(cw), nullptr});
  return;
}
    ...
    if (inst->opcode() == HloOpcode::kSubtract ||
      ...
      ExtractWeightedSlices(inst, combined_slices);
      {
        ...
        for (auto& ws : combined_slices) {
          ...
          if (ws.array_weight) {
            CHECK_EQ(slice_shape, ws.array_weight->shape());
            // Canonicalize order for consistent folding
            if (CompareInstruction(ws.array_weight, ws.slice)) {
              std::swap(ws.slice, ws.array_weight);
            }
          }
        }
        ...
        for (auto& ws : combined_slices) {
          if (folded.empty()) {
            ...
            if (ws.slice == folded.back().slice &&
                ws.array_weight == folded.back().array_weight) {
              HloEvaluator evaluator;
              folded.back().weight =
                  evaluator
        ...
        if (folded.size() == 1) {
          ...
          if (folded[0].array_weight != nullptr) {
            HloInstruction* replacement =
                computation->AddInstruction(HloInstruction::CreateBinary(
                    inst->shape(), HloOpcode::kMultiply, folded[0].slice,
                    folded[0].array_weight));
            TF_RETURN_IF_ERROR(
        ...
        for (auto& ws : folded) {
          ...
          if (any_array_weights) {
            HloInstruction* aw = ws.array_weight;
            if (!aw) {
              ...
              aw = computation->AddInstruction(
                  HloInstruction::CreateBroadcast(slice_shape, scalar, {}));
            ...
            reshaped_array_weights.push_back(computation->AddInstruction(
        ...
        if (any_array_weights) {
          ...
          replacement = computation->AddInstruction(HloInstruction::CreateDot(
              inst->shape(), concat, concat_weights, dnums, precision_config));
          TF_RETURN_IF_ERROR(
              computation->ReplaceInstruction(inst, replacement));
          changed = true;
          continue;
              ...
              [&](auto primitive_type_constant) -> Literal {
                if constexpr (primitive_util::IsArrayType(
                  ...
                  if constexpr (primitive_type_constant == PRED) {
                    std::unique_ptr<bool[]> bool_vals(new bool[folded.size()]);
                    for (size_t i = 0; i < folded.size(); ++i) {
                      bool_vals[i] = folded[i].weight.GetFirstElement<bool>();
                    }
                    return LiteralUtil::CreateR1<bool>(
                        absl::Span<const bool>(bool_vals.get(), folded.size()));
                  } else {
                    using NativeT =
          ...
          TF_CHECK_OK(computation->ReplaceInstruction(inst, replacement));
          changed = true;
          continue;
        }
      }
