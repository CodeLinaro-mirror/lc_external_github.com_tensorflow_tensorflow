...
absl::StatusOr<bool> TryHoistSliceThroughTranspose(
  ...
  HloInstruction* transpose = instruction->mutable_operand(0);
  if (transpose->opcode() != HloOpcode::kTranspose) {
    return false;
  }
  if (instruction->shape().element_type() !=
      transpose->shape().element_type()) {
    VLOG(1) << "Slice element type does not match transpose element type: "
  ...
  std::vector<int64_t> new_slice_starts =
      Permute(instruction->slice_starts(), inversed_dimensions_permutation);
  std::vector<int64_t> new_slice_limits =
      Permute(instruction->slice_limits(), inversed_dimensions_permutation);
  std::vector<int64_t> new_slice_strides =
      Permute(instruction->slice_strides(), inversed_dimensions_permutation);

  HloInstruction* transpose_operand = transpose->mutable_operand(0);
  Shape new_slice_shape = ShapeUtil::PermuteDimensions(
      inversed_dimensions_permutation, instruction->shape());
  ...
  HloInstruction* new_transpose =
      computation->AddInstruction(HloInstruction::CreateTranspose(
          instruction->shape(), new_slice, dimensions_permutation));
  TF_RETURN_IF_ERROR(
      computation->ReplaceInstruction(instruction, new_transpose));
  return true;
}
...
absl::StatusOr<bool> TryHoistingSlice(HloInstruction* instruction,
  ...
  for (auto hoisting_function : hoisting_functions) {
    bool changed = false;
    TF_ASSIGN_OR_RETURN(changed, hoisting_function(instruction, computation));
    if (changed) {
      return true;
    }
  }
