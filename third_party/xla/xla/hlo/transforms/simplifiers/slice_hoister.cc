  ...
  do {
    ...
    for (HloInstruction* instruction : instructions) {
      if (instruction->opcode() == HloOpcode::kSlice &&
        ...
        if (lhs->shape() != instruction->shape()) {
          VLOG(1) << " Slice shape does not match operand shape: "
                  << lhs->shape() << " and " << instruction->shape();
          continue;
        }
        if (instruction->shape() != add->shape()) {
          VLOG(1) << " Slice shape does not match add shape: "
                  << instruction->shape() << " and " << add->shape();
          continue;
        }
