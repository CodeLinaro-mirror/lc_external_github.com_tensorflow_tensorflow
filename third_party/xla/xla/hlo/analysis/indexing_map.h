...
class IndexingMap {
 public:
  ...
  mlir::AffineMap GetAffineMap() const {
    return SymbolicMapToAffineMap(GetSymbolicMap(), GetMLIRContext());
  }
  ...
  llvm::MapVector<SymbolicExpr, Interval> GetSymbolicConstraints() const {
    return constraints_;
  }
  ...
  bool IsRangeVarSymbol(SymbolicExpr symbol) const;

  // Does `symbol` correspond to an RTVar?
  bool IsRTVarSymbol(SymbolicExpr symbol) const;
