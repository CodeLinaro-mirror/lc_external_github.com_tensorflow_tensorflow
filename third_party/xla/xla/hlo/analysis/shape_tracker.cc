...
std::pair<ShapeTracker::Step, std::vector<int64_t>> ExpandReshapeStep(
  ...
  std::vector<int64_t> strides_to_preserve;
  std::inclusive_scan(step.dimensions.begin(), step.dimensions.end(),
                      std::back_inserter(strides_to_preserve),
                      std::multiplies<int64_t>(), 1);

  std::vector<int64_t> strides_to_insert;
  strides_to_insert.reserve(current_shape.size());
  std::inclusive_scan(current_shape.begin(), current_shape.end(),
                      std::back_inserter(strides_to_insert),
                      std::multiplies<int64_t>(), 1);
