...
AliasInfo::GetFusionInstructionInPlaceInputOutputPairs(
  ...
  auto follow_indirections = [&](const HloInstruction* instruction,
                                 ShapeIndex index) {
    const HloInstruction* prev_instruction = nullptr;
    ShapeIndex prev_index;
    while (instruction != prev_instruction || index != prev_index) {
      prev_instruction = instruction;
      prev_index = index;
      std::tie(instruction, index) = FollowTupleIndirection(instruction, index);
      std::tie(instruction, index) = FollowNoOpIndirection(instruction, index);
    }
