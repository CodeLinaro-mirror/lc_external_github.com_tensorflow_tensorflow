...
bool HloDimensionAnalysis::IsInstructionDotDependent(
    const HloInstruction* instruction) const {
  auto it = info_map_.find(instruction);
  if (it == info_map_.end()) {
    return false;
...
bool HloDimensionAnalysis::InstructionHasNontrivialDimensionInfo(
    const HloInstruction* instruction) const {
  auto it = info_map_.find(instruction);
  if (it == info_map_.end()) {
    return false;
...
absl::Status HloDimensionAnalysis::SetInstructionAs(HloInstruction* instruction,
                                                    DimensionInfo value) {
  ...
  dim_info_tree.ForEachMutableElement(
      [&](const ShapeIndex& index, DimensionInfo* dimension_info) {
        ...
        *dimension_info = DimensionInfo::kTuple;
      });
...
absl::Status HloDimensionAnalysis::AnnotateEntryComputationParameters(
  ...
  for (HloInstruction* instruction : params) {
    TF_RETURN_IF_ERROR(SetInstructionAs(instruction, DimensionInfo::kWeight));
  }
  return absl::OkStatus();
}
...
bool HloDimensionAnalysis::IsDotOrHasDotDependent(
  ...
  return IsInstructionDotDependent(op);
}
...
absl::Status HloDimensionInfoPropagation::HandleTuple(HloInstruction* tuple) {
  ...
  for (int64_t idx = 0; idx < tuple->operand_count(); ++idx) {
    ...
    if (analysis_->InstructionHasNontrivialDimensionInfo(operand)) {
...
absl::Status HloDimensionInfoPropagation::HandleSimpleOp(HloInstruction* op) {
  ...
  if (analysis_->IsInstructionWeight(operand)) {
    TF_RETURN_IF_ERROR(analysis_->SetInstructionAs(op, DimensionInfo::kWeight));
  } else if (analysis_->IsDotOrHasDotDependent(operand)) {
    TF_RETURN_IF_ERROR(
        analysis_->SetInstructionAs(op, DimensionInfo::kDotDependent));
  }
  return absl::OkStatus();
}
...
absl::Status HloDimensionInfoPropagation::HandleDynamicUpdateSlice(
  ...
  if (analysis_->IsInstructionWeight(operand) ||
    ...
    TF_RETURN_IF_ERROR(analysis_->SetInstructionAs(dynamic_update_slice,
                                                   DimensionInfo::kWeight));
  } else if (analysis_->IsInstructionDotDependent(operand) ||
             analysis_->IsInstructionDotDependent(update)) {
    TF_RETURN_IF_ERROR(analysis_->SetInstructionAs(
...
absl::Status HloDimensionInfoPropagation::HandleOptimizationBarrier(
  ...
  if (analysis_->InstructionHasNontrivialDimensionInfo(
          optimization_barrier_operand)) {
    TF_RETURN_IF_ERROR(analysis_->SetDimensionInfo(
