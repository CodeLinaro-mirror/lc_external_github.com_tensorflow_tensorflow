...
int CompareExprs(SymbolicExpr a, SymbolicExpr b) {
  // TODO(karupayun): Implement a proper comparison inside SymbolicExpr.
  if (a == b) {
    return 0;
  }
  return a < b ? -1 : 1;
...
SymbolicExpr SymbolicExprSimplifier::RewriteSum(SymbolicExpr sum) {
  ...
  llvm::sort(expanded_summands, [](SymbolicExpr a, SymbolicExpr b) {
...
IndexingMap::IndexingMap(
  ...
  llvm::MapVector<mlir::AffineExpr, Interval> constraints_map;
  for (const auto& [expr, range] : constraints) {
    constraints_map[expr] = range;
  }
  for (const auto& [expr, range] :
       ConvertAffineConstraintsToSymbolicConstraints(constraints_map,
  ...
  for (auto& [expr, range] : constraints_) {
    int64_t expr_value =
        expr.ReplaceDimsAndSymbols(dim_const_exprs, symbol_const_exprs,
                                   symbolic_map_.GetNumDims())
            .Evaluate({});  // TODO: karupayun - I changed that because I think
                            // it was a bug.
...
SmallVector<int64_t, 4> IndexingMap::Evaluate(
  ...
  llvm::errs() << "eval: ";
  for (const auto& expr : eval.GetResults()) {
    llvm::errs() << expr << " ";
  }
  llvm::errs() << "\n";
...
Interval RangeEvaluator::ComputeExpressionRange(SymbolicExpr expr) {
  ...
  if (use_constraints_) {
    auto constraint = indexing_map_.GetSymbolicConstraints().find(expr);
    if (constraint != indexing_map_.GetSymbolicConstraints().end()) {
...
bool IndexingMap::CompressVars(const llvm::SmallBitVector& unused_dims,
  ...
  std::vector<std::pair<SymbolicExpr, Interval>> to_add;
  for (const auto& [expr, range] : constraints_) {
    auto updated_expr = expr.ReplaceDimsAndSymbols(
        dim_replacements, symbol_replacements, num_dims_after);
...
IndexingMap ConvertRangeVariablesToDimensions(
  ...
  int64_t new_dims = symbolic_map.GetNumDims() + range_var_indices.size();
  int64_t new_symbols = symbolic_map.GetNumSymbols() - range_var_indices.size();
  // Update the symbolic map and the variables.
  std::vector<IndexingMap::Variable> dims = map.GetDimVars();
  std::vector<IndexingMap::Variable> range_vars;
  SmallVector<SymbolicExpr> symbol_replacements;
  symbol_replacements.reserve(symbolic_map.GetNumSymbols());
  ...
  for (int i = 0; i < map.GetRangeVarsCount(); ++i) {
    ...
    if (updated_count < range_var_indices_count &&
      ...
      symbol_replacements.push_back(CreateDimExpr(
          symbolic_map.GetNumDims() + updated_count, mlir_context));
      dims.push_back(range_var);
      updated_count++;
    } else {
      symbol_replacements.push_back(
          CreateSymbolExpr(i - updated_count, new_dims, mlir_context));
  ...
  SymbolicMap converted_symbolic_map = symbolic_map.ReplaceDimsAndSymbols(
      ...
      /*num_result_dims=*/new_dims,
      /*num_result_symbols=*/new_symbols);
  ...
  for (auto constraint : map.GetSymbolicConstraints()) {
    constraints.push_back(
        {constraint.first.ReplaceSymbols(symbol_replacements, new_dims),
         constraint.second});
  }
