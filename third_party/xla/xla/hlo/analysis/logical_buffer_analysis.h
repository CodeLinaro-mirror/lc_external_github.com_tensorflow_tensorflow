...
//
// This class visits HLO instructions in DFS order to determine which output
// shape indices of which HLO instructions define/create a new buffer and which
// ones alias existing buffers. It assigns a unique `LogicalBuffer::Id` to each
...
class LogicalBufferAnalysis : public DfsHloVisitorWithDefault {
 public:
  ...
  // Otherwise, this method throws a `std::out_of_range` exception.
 ...
 private:
  ...
  // Flag indicating if dataflow relations (like custom call/fusion output to
  // operand aliasing) can be ignored so that separate buffers are created
  // instead of aliasing. This is currently disabled.
