...
struct MyExtension {
  ...
  // For this example the extension type for both C/C++ API is the same.
  ...
  static Context Instantiate(const CExtension* ext) { return Context{ext}; }
...
TEST(FfiTest, DecodeExtension) {
  ...
  auto handler = Ffi::Bind().Ctx<Extension<MyExtension>>().To(
      [&](const MyExtension::Context ctx) -> absl::Status {
...
TEST(FfiTest, DecodeExtensionNotFound) {
  ...
  auto handler = Ffi::Bind().Ctx<Extension<AnotherExtension>>().To(
      [&](const AnotherExtension::Context ext) -> absl::Status {
        handler_called = true;
        return absl::OkStatus();
      });
