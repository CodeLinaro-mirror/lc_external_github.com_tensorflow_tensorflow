...
TEST_F(HloShardingTest, ToStringWithNamedShardingTest) {
  ...
  NamedSharding::DimensionSharding ds_a({AxisRef(0)},
                                        /*is_closed=*/true);
  HloSharding sharding(NamedSharding(mesh, {ds_a}));
  EXPECT_EQ(sharding.ToString(), "{@mesh<a=2,b=4>, [{a}]}");

  HloSharding sharding_metadata(
      NamedSharding(mesh, {ds_a}, {}, {}, ListMetadata()));
  EXPECT_EQ(
      sharding_metadata.ToString(/*include_metadata=*/true),
      "{@mesh<a=2,b=4>, [{a}], metadata={{op_name=\"b\"}, {op_name=\"c\"}}}");

  HloSharding tuple_sharding(HloSharding::Tuple(
      ShapeUtil::MakeTupleShape({ShapeUtil::MakeShape(F32, {3, 5}),
                                 ShapeUtil::MakeShape(U32, {7, 25}),
                                 ShapeUtil::MakeShape(S32, {9, 11})}),
      {sharding, sharding, sharding_metadata}));
  EXPECT_EQ(
      tuple_sharding.ToString(/*include_metadata=*/true),
      "{{@mesh<a=2,b=4>, [{a}]}, {@mesh<a=2,b=4>, [{a}]}, {@mesh<a=2,b=4>, "
      "[{a}], metadata={{op_name=\"b\"}, {op_name=\"c\"}}}}");
}
