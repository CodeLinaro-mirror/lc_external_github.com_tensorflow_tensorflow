...
void AppendToWhileLoopOriginalValue(
    HloInstruction* while_instr,
    const HloInstruction::InstructionVector& new_while_input_operands) {
  auto append_to_original_value = [&](HloInstruction* instr,
    ...
    if (old_original_value == nullptr) {
      ...
      std::for_each(
          new_while_input_operands.begin(), new_while_input_operands.end(),
    ...
    for (int64_t i = 0; i < new_while_input_operands.size(); ++i) {
      if (new_while_input_operands[i]->original_value() != nullptr) {
        new_original_value->mutable_tree()->CopySubtreeFrom(
            new_while_input_operands[i]->original_value()->tree(), {},
            {next_index + i});
      }
    }
  ...
  // value.
  int64_t next_index =
      while_shape.tuple_shapes().size() - new_while_input_operands.size();
