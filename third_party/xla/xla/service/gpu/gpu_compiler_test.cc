...
class GpuCompilerTestWithAutotuneDb : public GpuCompilerTest {
 public:
  void GenerateAutotuneDbWithoutTriton(const absl::string_view hlo_string) {
    AutotunerCache::ClearAutotuneResults();
    // Generate the autotuner db without Triton as a possible backend.
    HloModuleConfig config;
    DebugOptions debug_options = GetDebugOptionsForTest();
    ...
    EXPECT_FALSE(absl::StrContains(results.DebugString(), "TRITON"));
  }
...
ENTRY main {
  ...
  // Generate an autotuning database without Triton; this will force the
  // triton_enabled_module to use cuBLAS.
  GenerateAutotuneDbWithoutTriton(hlo_string);
  HloModuleConfig config;

  // Triton enabled, but force to fallback to cuBLAS.
  DebugOptions triton_enabled_debug_options = GetDebugOptionsForTest();
  // This confirms that the autotuner will fallback to cublas rather than choose
  //  Triton because we already generated the autotune database without Triton.
...
ENTRY main {
  ...
  HloModuleConfig config;
  // Triton enabled, but forced to fallback to cuBLAS.
  DebugOptions triton_enabled_debug_options = GetDebugOptionsForTest();
  triton_enabled_debug_options
      .set_xla_gpu_require_complete_aot_autotune_results(true);
  config.set_debug_options(triton_enabled_debug_options);
