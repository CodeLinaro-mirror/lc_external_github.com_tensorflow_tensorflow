...
TEST_F(GpuDotFusionCostModelTest, CalculateHardwareLaunchWaves) {
  // Case 1: Zero threadblocks should require zero waves.
  EXPECT_EQ(0,
            CalculateHardwareLaunchWaves(/*threadblock_count=*/0,
                                         /*shared_memory_per_block_bytes=*/1024,
                                         /*num_warps=*/4, ddh100_));

  // Case 2: Small shared memory with few threadblocks should require 1 wave.
  ...
  // Case 3: Large shared memory should require more waves to execute the same
  // number of blocks.
  ...
  // Case 4: More threadblocks requires more waves.
...
TEST_F(GpuDotFusionCostModelTest, CalculatePipelinedLoopTimeWithLaunchWaves) {
  HbmEstimates hbm_timing;
  hbm_timing.read_time = absl::Microseconds(100);
  hbm_timing.write_time = absl::Microseconds(50);
  absl::Duration compute_time = absl::Microseconds(200);
  ...
  // Case 2: Wave boundary execution overhead. Many waves should be slower than
