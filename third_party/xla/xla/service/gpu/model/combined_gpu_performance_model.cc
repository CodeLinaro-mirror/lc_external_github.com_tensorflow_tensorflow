...
absl::Status CombinedGpuPerformanceModel::Invalidate(
    const HloComputation& computation, const HloInstruction& instruction) {
  if (instruction.parent() != &computation && instruction.parent() != nullptr) {
    return absl::InvalidArgumentError(
        "Cannot invalidate instruction attached to unexpected computation");
  }

  absl::MutexLock lock(cache_mutex_);
  // instruction.unique_id() crashes when called on an instruction that was
  // already detached from its computation. To allow invalidating instructions
  // post-fusion, leverage explicitly passed computation.
  fusion_analysis_cache_.Invalidate(HloInstruction::CalculateUniqueId(
      computation.unique_id(), instruction.local_id()));
