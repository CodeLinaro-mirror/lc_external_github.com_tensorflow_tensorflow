...
int64_t GetRegionMultiplier(const TiledHloInstruction* tiled_hlo) {
  ...
  if (HloPredicateIsOp<HloOpcode::kDot, HloOpcode::kScaledDot>(hlo)) {
    ...
    for (int64_t dim : contracting_dimensions) {
      multiplier *= llvm::divideCeil(hlo->operand(0)->shape().dimensions(dim),
...
// The `visitor` function is called with each `TiledHloInstruction`.
void ForEachInstructionInTiledHloComputation(
    const TiledHloComputation& tiled_hlo_computation,
    absl::FunctionRef<void(const TiledHloInstruction*)> visitor) {
...
bool DoesComputationFitInRegisters(
  ...
  bool fits = true;
  ForEachInstructionInTiledHloComputation(
      tiled_hlo_computation, [&](const TiledHloInstruction* tiled_hlo) {
        if (!fits) {
          return;
        }
  ...
  ForEachInstructionInTiledHloComputation(
      ...
      [&](const TiledHloInstruction* tiled_hlo, int64_t multiplier) {
        ...
        if (fusion_adaptor.ContainsInstruction(hlo)) {
          ...
          // Total number of elements that are read from memory across all
          // blocks.
          //
          // Triton requires that all tiles have dimensions that are padded to
          // the next power of 2. However, the load masks the padded elements,
          // so they are not read from memory, but set directly in registers. As
          // a result, the number of elements read from memory is equal to the
          ...
          // to be since it'll end up assuming that if one read is done with
          // lower bandwidth, all other reads of the same operand will also be
          // done with lower bandwidth. But it's a start. We should refactor
          // this function to properly track each read independently later.
