...
class GemmFusionVisitor : public DfsHloRewriteVisitor {
 public:
  ...
  absl::Status HandleScaledDot(HloInstruction* scaled_dot) override {
    ...
    std::vector<HloInstruction*> new_operands;
    new_operands.reserve(scaled_dot->operand_count());
    new_operands.push_back(create_parameter(0, "lhs"));
    new_operands.push_back(create_parameter(1, "lhs_scale"));
    new_operands.push_back(create_parameter(2, "rhs"));
    new_operands.push_back(create_parameter(3, "rhs_scale"));
    builder.AddInstruction(
    ...
    FusionBackendConfig& backend_config =
        *gpu_config.mutable_fusion_backend_config();
    backend_config.set_kind("__triton_scaled_dot");
