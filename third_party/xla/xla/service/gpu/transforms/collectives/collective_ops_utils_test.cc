...
bool IsCombineCollective(se::CudaComputeCapability compute_capability,
  ...
  return xla::gpu::IsCombineCollective(config, device_description,
                                       nvlink_slice_size);
}

TEST(IsCombineCollectiveTest, SingleHostSingleDevice) {
  // B200
  EXPECT_FALSE(IsCombineCollective(se::CudaComputeCapability::Blackwell(),
                                   /*num_partitions=*/1, /*replica_count=*/1,
                                   /*nvlink_slice_size=*/8));
  // H100
  EXPECT_FALSE(IsCombineCollective(se::CudaComputeCapability::Hopper(),
                                   /*num_partitions=*/1, /*replica_count=*/1,
                                   /*nvlink_slice_size=*/8));
  // A100
  EXPECT_FALSE(IsCombineCollective(se::CudaComputeCapability::Ampere(),
                                   /*num_partitions=*/1, /*replica_count=*/1,
                                   /*nvlink_slice_size=*/16));
}

TEST(IsCombineCollectiveTest, SingleHostMultiDevices) {
  // B200
  EXPECT_FALSE(IsCombineCollective(se::CudaComputeCapability::Blackwell(),
...
TEST(IsCombineCollectiveTest, MultiHosts) {
  // B200
  EXPECT_TRUE(IsCombineCollective(se::CudaComputeCapability::Blackwell(),
                                  /*num_partitions=*/16, /*replica_count=*/1,
                                  /*nvlink_slice_size=*/8));
  EXPECT_TRUE(IsCombineCollective(se::CudaComputeCapability::Blackwell(),
                                  /*num_partitions=*/1, /*replica_count=*/16,
                                  /*nvlink_slice_size=*/8));
  // H100
  EXPECT_TRUE(IsCombineCollective(se::CudaComputeCapability::Hopper(),
                                  /*num_partitions=*/16, /*replica_count=*/1,
                                  /*nvlink_slice_size=*/8));
  EXPECT_TRUE(IsCombineCollective(se::CudaComputeCapability::Hopper(),
                                  /*num_partitions=*/1, /*replica_count=*/16,
                                  /*nvlink_slice_size=*/8));
  // A100
  EXPECT_TRUE(IsCombineCollective(se::CudaComputeCapability::Ampere(),
...
TEST(IsCombineCollectiveTest, UnsupportedGPU) {
  EXPECT_FALSE(IsCombineCollective(se::CudaComputeCapability::Volta(),
                                   /*num_partitions=*/1, /*replica_count=*/1,
                                   /*nvlink_slice_size=*/8));
}
