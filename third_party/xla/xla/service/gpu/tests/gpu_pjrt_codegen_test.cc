...
void GpuPjRtCodegenTest::CompileAndVerifyIr(const std::string& hlo_text,
                                            const std::string& expected_llvm_ir,
                                            bool match_optimized_ir,
                                            bool run_optimization_passes) {
  TF_ASSERT_OK_AND_ASSIGN(std::unique_ptr<HloModule> hlo_module,
                          ParseAndReturnVerifiedModule(hlo_text));
  auto llvm_compiler = static_cast<LLVMCompiler*>(compiler());
  TF_EXPECT_OK(xla::CompileAndVerifyIr(
