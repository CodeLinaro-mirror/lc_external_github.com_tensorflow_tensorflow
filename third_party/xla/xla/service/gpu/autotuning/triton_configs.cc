...
const std::vector<TritonGemmConfig>& GetTritonConfigsForPlatform(
  ...
  static const absl::NoDestructor<
      ...
      kConfigs({{TritonConfigsPlatform::kAmpere, ParseConfig(get_a100())},
                {TritonConfigsPlatform::kBlackwell, ParseConfig(get_b200())},
                {TritonConfigsPlatform::kDefaultCuda, ParseConfig(get_cuda())},
                {TritonConfigsPlatform::kDefaultRocm, ParseConfig(get_rocm())},
                {TritonConfigsPlatform::kHopper, ParseConfig(get_h100())}});
