...
TEST_F(GpuCopyInsertionTest, InterleavedTupleNoOpFusedDynamicUpdateSliceCopy) {
  constexpr absl::string_view kModuleString = R"(
HloModule Module
...
fused_computation {
  ...
  constant.0 = s32[] constant(0)
  dus = f32[4,4] dynamic-update-slice(param0, update, constant.0, constant.0)
  bitcast = f32[16] bitcast(dus)
  tuple = (f32[16], f32[4,4]) tuple(bitcast, param0)
  ROOT gte = f32[16] get-tuple-element(tuple), index=0
}
