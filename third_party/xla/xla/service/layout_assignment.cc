...
bool BufferLayoutConstraint::UpdateLayout(int64_t priority,
  ...
  priority_ = priority;
  from_user_ = user;
  Layout merged_layout = new_layout;
  if (layout().memory_space() != Layout::kDefaultMemorySpace &&
      new_layout.memory_space() == Layout::kDefaultMemorySpace) {
    merged_layout.set_memory_space(layout().memory_space());
  }
  layout_.push_back(layout());
  layout_[0] = merged_layout;
  return true;
}
...
absl::Status LayoutAssignment::SetBufferLayout(const Layout& layout,
  ...
  if (layout.minor_to_major().empty() && !buffer.shape().dimensions().empty()) {
    VLOG(3) << "Ignore partial layout constraint: "
  ...
  ABSL_RETURN_IF_ERROR(ShapeUtil::ForEachSubshapeWithStatus(
      ...
      [this, dfs, instruction, mandatory, allow_alias, priority,
       subshape_index](const Shape& subshape,
        ...
        if (subshape.IsArray() &&
            LayoutUtil::HasNonMemorySpaceLayout(subshape)) {
...
absl::Status LayoutAssignment::AddParameterConstraints(
  ...
  if (reverse_computation_order_ ||
      (constraints->computation()->IsEntryComputation() &&
       entry_computation_layout_->AnyNonMemorySpaceLayoutSet()) ||
    ...
    if (parameter_layout.AnyNonMemorySpaceLayoutIsSet()) {
...
bool LayoutAssignment::PropagateOperandLayoutToAsyncParameter(
  ...
  if (param_idx >= async_layout->parameter_count() ||
      !async_layout->parameter_layout(param_idx).NonMemorySpaceLayoutIsSet()) {
    return false;
  }
...
bool LayoutAssignment::PropagateResultLayoutToAsyncSubComputation(
  ...
  if (!instruction->shape().IsTuple() ||
      ...
      !async_layout->result_layout().NonMemorySpaceLayoutIsSet()) {
    return false;
  }
...
absl::Status LayoutAssignment::AddAsyncDoneConstraints(
  ...
  } else if (async_layout.result_layout().NonMemorySpaceLayoutIsSet()) {
...
absl::Status LayoutAssignment::CheckLayouts(
  ...
  for (auto* computation :
    ...
    for (auto* instruction : computation->instructions()) {
      ...
      auto status = ShapeUtil::ValidateShape(instruction->shape());
      if (!status.ok()) {
        LOG(ERROR) << "CheckLayouts failed for instruction: "
                   << instruction->ToString() << ", shape: "
  ...
  // computation root.
  const ShapeLayout& result_layout = entry_computation_layout_->result_layout();
...
absl::Status LayoutAssignment::AssignLayouts(LayoutConstraints& constraints) {
  ...
  if (constraints.ResultLayout() != nullptr &&
      constraints.ResultLayout()->NonMemorySpaceLayoutIsSet()) {
...
absl::Status LayoutAssignment::PropagateComputationLayouts(
  ...
  for (int64_t i = 0; i < computed_computation_layout.parameter_count(); ++i) {
    ...
    ABSL_RETURN_IF_ERROR(ShapeUtil::ForEachSubshapeWithStatus(
        ...
        [&](const Shape& subshape,
          ...
          if (!LayoutUtil::HasNonMemorySpaceLayout(subshape)) {
            needs_assign = true;
            return absl::OkStatus();
          }
  ...
  if (!result_layout->NonMemorySpaceLayoutIsSet()) {
