...
std::vector<LiveRange> LiveRangeCalculator::DetermineLiveRangesGivenLastUses(
  ...
  if (last_inst->opcode() == HloOpcode::kWhile) {
    ...
    // An HloValue that is an operand to a while loop, at one or more operand
    // indices, is a pass-through, if it is a read only operand inside the while
    // loop. If the last uses at the while loop is a pass-through, the live
    // range end would be until the while instruction.
