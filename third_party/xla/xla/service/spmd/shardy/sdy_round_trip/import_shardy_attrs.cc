...
// Returns true if the transition from callerSharding to calleeConstraint
// contains a replicated-to-partitioned (slicing) transition in any dimension.
// Slicing requires partition-id() instructions on SparseCore, which are
// unsupported and crash the compiler.
bool isSlicing(TensorShardingAttr callerSharding,
               TensorShardingAttr calleeConstraint) {
  if (!calleeConstraint) {
    return false;
  }
  if (!callerSharding) {
    ...
    // layouts, avoiding a slicing transition at the boundary.
    return llvm::any_of(calleeConstraint.getDimShardings(),
                        [](auto dim) { return !dim.emptyAxes(); });
  }

  auto callerDims = callerSharding.getDimShardings();
  auto calleeDims = calleeConstraint.getDimShardings();
...
// it inherits the layout constraint directly, ensuring they match at the
// boundary and avoiding slicing entirely.
void insertPropagationBarriers(FuncOp funcOp, ModuleOp moduleOp,
  ...
  for (const mlir::BlockArgument& arg : entryBlock.getArguments()) {
    auto useIt = llvm::find_if(arg.getUses(), [](const mlir::OpOperand& use) {
    ...
    if (useIt != arg.getUses().end()) {
      const auto customCallOp = mlir::cast<CustomCallOp>(useIt->getOwner());
      TensorShardingAttr calleeConstraint =
          getSharding(customCallOp->getResult(0));
      bool shouldInsertBarrier = true;
      if (calleeConstraint) {
        if (isSlicingAtCallSites(funcOp, arg.getArgNumber(), calleeConstraint,
                                 moduleOp)) {
