...
class ImportFuncCallsPass
 ...
 public:
  ...
  ImportFuncCallsPass(ImportFuncCallsPassOptions options)
      : ImportFuncCallsPass() {
    onlyUninlineable = std::move(options.onlyUninlineable);
  }
...
std::unique_ptr<mlir::Pass> createImportFuncCallsPass(
  ...
  return std::make_unique<ImportFuncCallsPass>(options);
}

void registerImportFuncCallsPass() {
  mlir::registerPass([] {
    return createImportFuncCallsPass(
        ImportFuncCallsPassOptions{/*onlyUninlineable=*/true});
  });
}
