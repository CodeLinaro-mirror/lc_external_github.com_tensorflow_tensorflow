...
std::optional<CollectiveDeviceList> TryExpandingPartitionGroupList(
  ...
  if (device_list.version() != CollectiveDeviceListVersion::kIota &&
      device_list.version() != CollectiveDeviceListVersion::kMeshAxes) {
    return std::nullopt;
  }

  auto iota_list = device_list.MaybeConvertToIotaReplicaGroupList();
  ...
  SPMDCollectiveOpsCreator result = {
      ...
      .create_all_reduce =
          [num_replicas, num_partitions](
              ...
              HloComputation* reduction,
              const CollectiveDeviceListBase& device_list, int64_t channel_id) {
            if (auto expanded = TryExpandingPartitionGroupList(
                    device_list, num_replicas, num_partitions)) {
              HloComputation* reduction_clone =
