...
absl::StatusOr<bool> SpmdPartitioner::RunImpl(
  ...
  if (options_.allow_module_signature_change) {
    ...
    for (int64_t i = 0; i < new_program_shape.parameters_size(); ++i) {
      const Shape& old_layout =
          module->entry_computation_layout().parameter_shape(i);
      TF_RETURN_IF_ERROR(LayoutUtil::CopyLayoutBetweenShapes(
          old_layout, new_program_shape.mutable_parameters(i)));
      ShapeUtil::ForEachMutableSubshape(
          new_program_shape.mutable_parameters(i),
          [this, &old_layout](Shape* subshape, const xla::ShapeIndex& index) {
            if (subshape->IsArray() && subshape->has_layout() &&
                !Shape::Equal().IgnoreLayout()(
                    *subshape, ShapeUtil::GetSubshape(old_layout, index))) {
              UpdateLayout(subshape);
            }
          });
    }

    const Shape& old_layout = module->entry_computation_layout().result_shape();
    TF_RETURN_IF_ERROR(LayoutUtil::CopyLayoutBetweenShapes(
        old_layout, new_program_shape.mutable_result()));
    ShapeUtil::ForEachMutableSubshape(
