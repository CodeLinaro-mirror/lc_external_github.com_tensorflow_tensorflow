...
void EventForest::ConnectIntraThreadTPU(XPlane* plane, XPlaneVisitor* visitor,
  ...
  // Step 3: Store the parent nodes for later if they fail to be grouped with
  // the host events.
  // NOTE: SparseCore and TensorCore are split to ensure the TensorCore parent
  // events are grouped first so that the SparseCore events can be grouped
  // relative to the TensorCore events.
  if (tc_id.has_value()) {
