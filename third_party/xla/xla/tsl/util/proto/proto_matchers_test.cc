...
TEST(EqualsProto, DescribesSelfWhenGivenString) {
  ...
  EXPECT_EQ(Describe(matcher),
            R"pb(equals s1: "foo" r3: "a" r3: "b" r3: "c" s4: "t")pb");
  EXPECT_EQ(Describe(Not(matcher)),
            R"pb(not equals s1: "foo" r3: "a" r3: "b" r3: "c" s4: "t")pb");
}
...
TEST(EquivToProto, DescribesSelfWhenGivenProto) {
  ...
  EXPECT_THAT(
      Describe(matcher),
      MatchesRegex("equates (.|\n)*s1: \"foo\"(.|\n)*r3: \"a\"(.|\n)r3: "
                   "\"b\"(.|\n)r3: \"c\"(.|\n)s4: \"t\"(.|\n)"));
  EXPECT_THAT(
      Describe(Not(matcher)),
      MatchesRegex("not equates (.|\n)*s1: \"foo\"(.|\n)*r3: "
                   "\"a\"(.|\n)r3: \"b\"(.|\n)r3: \"c\"(.|\n)s4: \"t\"(.|\n)"));
}

TEST(EquivToProto, DescribesSelfWhenGivenString) {
  const Matcher<const Foo&> matcher =
      EquivToProto(R"pb(s1: "foo" r3: "a" r3: "b" r3: "c" s4: "t")pb");

  EXPECT_EQ(Describe(matcher),
            R"pb(equates s1: "foo" r3: "a" r3: "b" r3: "c" s4: "t")pb");
  EXPECT_EQ(Describe(Not(matcher)),
            R"pb(not equates s1: "foo" r3: "a" r3: "b" r3: "c" s4: "t")pb");
}
