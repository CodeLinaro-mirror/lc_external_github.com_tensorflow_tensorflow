...
// Represents an executable for a single core that has been fully loaded and is
// ready to run. Note that this can only be invoked once, so it needs to be
// reloaded for every computation. This relies on code to handle performing the
// loading each time.
