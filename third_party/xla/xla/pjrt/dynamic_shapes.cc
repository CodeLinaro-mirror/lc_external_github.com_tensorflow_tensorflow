...
void ReadDynamicShape(PjRtRawBufferRef raw_buffer,
  ...
  size_t actual_alignment =
      std::max(requirements.metadata_alignment, sizeof(void*));
  if (posix_memalign(&scratch, actual_alignment, requirements.metadata_size) !=
      0) {
    output_shape.SetError(absl::InternalError("posix_memalign failed"));
    return;
  }
