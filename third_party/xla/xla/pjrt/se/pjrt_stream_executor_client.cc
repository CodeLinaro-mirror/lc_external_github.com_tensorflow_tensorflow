...
PjRtStreamExecutorClient::LinearizeHostBufferInto(
    ...
    if (staging_buffer) {
      RETURN_IF_ERROR(
          Linearize(absl::MakeSpan(static_cast<uint8_t*>(staging_buffer.get()),
                                   packed_size),
                    data, type, dims, *byte_strides, device_shape.layout(),
                    /*dynamic_sizes=*/{}));
    }
  ...
  auto transfer_h2d =
      [this, local_device, data, type,
       captured_dims =
           absl::InlinedVector<int64_t, 4>(dims.begin(), dims.end()),
       captured_strides = absl::InlinedVector<int64_t, 4>(byte_strides->begin(),
                                                          byte_strides->end()),
       device_layout = device_shape.layout(), packed_size,
       event = definition_event, raw_buffer,
          ...
          if (host_buffer_semantics !=
            ...
            CHECK_OK(Linearize(
                absl::MakeSpan(static_cast<uint8_t*>(staging_buffer.get()),
                               packed_size),
                data, type, captured_dims, captured_strides, device_layout,
                /*dynamic_sizes=*/{}));
          }
