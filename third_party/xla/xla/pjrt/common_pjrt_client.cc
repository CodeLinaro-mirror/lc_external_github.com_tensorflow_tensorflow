...
CommonPjRtLoadedExecutable::ExecutePortable(
  ...
  ASSIGN_OR_RETURN(auto result,
                   ExecuteHelperOnSingleDevice(argument_handles, /*replica=*/0,
                                               /*partition=*/0, options,
                                               fill_future, device));
  returned_future = std::move(result.future);
  return std::move(result.buffers);
...
CommonPjRtLoadedExecutable::Execute(
  ...
  tsl::profiler::TraceMe traceme("TpuLoadedExecutable::Execute");
  VLOG(1) << "TpuLoadedExecutable::Execute";
  ...
  tsl::profiler::TraceMeProducer producer("TpuLoadedExecutable::Execute",
                                          tsl::profiler::ContextType::kPjRt);
      ...
      for (int i = 0; i < num_addressable_devices; ++i) {
        ...
        LaunchOnDevice(device, [&, replica, partition, i, context_id] {
          tsl::profiler::TraceMeConsumer consumer(
              "Scheduled TpuLoadedExecutable::Execute",
