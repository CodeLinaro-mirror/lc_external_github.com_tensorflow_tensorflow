...
class PjRtBuffer {
 public:
  ...
  // Bitcast the buffer to a new buffer of the same on-device size but possibly
  // different `element_type`, `dims`, and `device_layout`. This is a
  // metadata-only operation and does not copy any data. The new buffer will
  // alias to the input buffer, sharing the same underlying device memory as the
  // original buffer.
