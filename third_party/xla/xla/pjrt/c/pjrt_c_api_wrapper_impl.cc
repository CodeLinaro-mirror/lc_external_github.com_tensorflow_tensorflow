...
PJRT_Error* PJRT_Executable_DeserializeAndLoad(
  ...
  if (args->struct_size >=
      PJRT_STRUCT_SIZE(PJRT_Executable_DeserializeAndLoad_Args,
                       load_options_computation_origin_size)) {
    if (args->load_options_computation_origin != nullptr) {
      load_options.computation_origin = xla::PjRtDeviceDimensions(
          absl::MakeSpan(args->load_options_computation_origin,
                         args->load_options_computation_origin_size));
    }
  }
  ...
  if (args->struct_size >=
      PJRT_STRUCT_SIZE(PJRT_Executable_DeserializeAndLoad_Args,
                       multi_slice_config)) {
    if (args->multi_slice_config != nullptr) {
      load_options.multi_slice_config = args->multi_slice_config->config.get();
    }
  }
