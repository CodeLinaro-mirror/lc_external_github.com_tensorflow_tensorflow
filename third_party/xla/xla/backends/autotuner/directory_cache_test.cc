...
TEST_F(DirectoryCacheTest, BasicInsertAndLookup) {
  ...
  AutotuneScope ctx;
  ctx.device = "device_1";
  ctx.explicit_version = "v1";
  ctx.codegen_version = "cg_v1";

  DirectoryCache cache(ctx, cache_dir_, CacheMode::kReadWrite,
  ...
  DirectoryCache cache2(ctx, cache_dir_, CacheMode::kReadOnly,
                        KeyMatchingMode::kStrict);
...
TEST_F(DirectoryCacheTest, EmptyExplicitVersionOmitTier) {
  ...
  AutotuneScope ctx;
  ctx.device = "device_1";
  ctx.explicit_version = "";  // Empty version
  ctx.codegen_version = "cg_v1";
...
TEST_F(DirectoryCacheTest, StrictModeRejection) {
  ...
  AutotuneScope ctx1;
  ctx1.device = "device1";
  ctx1.explicit_version = "v1.0";
  ctx1.codegen_version = "cg_v1";

  DirectoryCache cache1(ctx1, cache_dir_, CacheMode::kReadWrite,
                        KeyMatchingMode::kStrict);
  AutotunerCacheInterface::Config config = CreateTestConfig();
  EXPECT_OK(cache1.Insert(instr1, config));

  // Lookup with different codegen_version.
  AutotuneScope ctx2 = ctx1;
  ctx2.codegen_version = "cg_v2";
  DirectoryCache cache2(ctx2, cache_dir_, CacheMode::kReadOnly,
                        KeyMatchingMode::kStrict);
...
TEST_F(DirectoryCacheTest, LooseModeChecks) {
  ...
  ctx1.explicit_version = "v1.0";
  ctx1.codegen_version = "cg_v1";
  ctx1.per_backend_versions[autotuner::Backend::TRITON] = "triton_v1";
  ...
  AutotuneScope ctx3 = ctx1;
  ctx3.codegen_version = "cg_v2";
  ctx3.per_backend_versions[autotuner::Backend::TRITON] = "triton_v2";
  DirectoryCache cache3(ctx3, cache_dir_, CacheMode::kReadOnly,
                        KeyMatchingMode::kLoose);
...
TEST_F(DirectoryCacheTest, CacheModeConstraints) {
  ...
  AutotuneScope ctx;
  ctx.device = "device1";
  ctx.explicit_version = "v1.0";
  ctx.codegen_version = "cg_v1";

  // ReadOnly mode Insert should fail.
  DirectoryCache cache_ro(ctx, cache_dir_, CacheMode::kReadOnly,
                          KeyMatchingMode::kStrict);
  AutotunerCacheInterface::Config config = CreateTestConfig();
  ...
  DirectoryCache cache_rw(ctx, cache_dir_, CacheMode::kReadWrite,
                          KeyMatchingMode::kStrict);
  EXPECT_OK(cache_rw.Insert(instr1, config));

  // ReadAppend mode Insert of existing entry should fail.
  DirectoryCache cache_ra(ctx, cache_dir_, CacheMode::kReadAppend,
                          KeyMatchingMode::kStrict);
