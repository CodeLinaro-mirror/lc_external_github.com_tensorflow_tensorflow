...
absl::Status Autotuner::Autotune(HloModule* module,
  ...
  for (auto& [_, instructions] : instructions_by_fingerprint) {
    ...
    bool dump = autotune_config_.dump_hlos;
    for (auto* instr : instructions) {
      TF_RETURN_IF_ERROR(ApplyConfig(instr, config, dump));
      // Do not dump instructions with the same fingerprint more than once.
      dump = false;
    }
  }
...
absl::Status Autotuner::Autotune(HloModule* module,
  ...
  for (auto& [_, instructions] : all_instructions_by_fingerprint) {
    ...
    bool dump = autotune_config_.dump_hlos;
    for (auto* instr : instructions) {
      TF_RETURN_IF_ERROR(ApplyConfig(instr, *cached_config, dump));
      // Do not dump instructions with the same fingerprint more than once.
      dump = false;
    }
  }
...
absl::Status Autotuner::ApplyConfig(HloInstruction* instr, const Config& config,
                                    bool dump_module) {
  if (dump_module) {
    ...
    DumpToFileInDirOrStdout(*parent_module, "", absl::StrCat(id, ".before.txt"),
                            module->ToString());
    // Apply the config to the extracted module to dump the module. We do this
    // because Apply config can rewrite the instruction or change neighboring
    // instructions, making the instruction itself no longer representative.
    HloInstruction* root = module->entry_computation()->root_instruction();
    TF_RETURN_IF_ERROR(ApplyConfig(root, config, /*dump_module=*/false));
