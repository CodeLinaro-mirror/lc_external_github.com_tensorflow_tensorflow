...
TEST_F(AutotunerTest, ShardedAutotuning) {
  ...
  MultiProcessKeyValueStore sharding_kv_store(kv_store, 0, kShardCount);
  EXPECT_THAT(
...
TEST_F(AutotunerTest, DumpHlos) {
  auto dump_dir = tsl::io::JoinPath(tsl::testing::TmpDir(), "dump_dir");
  auto module = ParseAndReturnVerifiedModule(kHlo).value();
  module->mutable_config().mutable_debug_options().set_xla_dump_to(dump_dir);
  auto should_autotune = [](const HloInstruction& instruction) {
    return instruction.opcode() == HloOpcode::kCopy ||
  ...
  EXPECT_THAT(tsl::Env::Default()->GetChildren(dump_dir, &files), IsOk());
