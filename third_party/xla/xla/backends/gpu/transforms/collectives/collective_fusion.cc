...
std::vector<Candidate> GetFusionCandidates(
    HloModule* module,
...
absl::Status FuseCandidate(const Candidate& candidate,
                           const GpuTopology& gpu_topology,
                           const DeviceAssignment* device_assignment) {
  const bool should_flatten = ShouldFlatten(candidate.instr);
  HloFusionInstruction* fusion_instr =
      CreateCollectiveFusionInstruction(candidate.computation, candidate.instr,
                                        HloInstruction::FusionKind::kCustom);
  RETURN_IF_ERROR(candidate.instr->ReplaceAllUsesWith(fusion_instr));
  RETURN_IF_ERROR(candidate.computation->RemoveInstruction(candidate.instr));
  if (should_flatten) {
...
absl::StatusOr<bool> CollectiveFusion::RunImpl(
  ...
  std::vector<Candidate> candidates = GetFusionCandidates(
      module, execution_threads, instructions_to_fuse, *device_assignment);
