...
bool AllowedInGemmFusion(const HloInstruction& instr) {
  ...
  return !HloPredicateIsOp<HloOpcode::kFusion, HloOpcode::kDot,
...
Fusion CreateFusionSpace(HloDotInstruction& dot,
                         const se::GpuComputeCapability& gpu_version) {
  HloComputation::Builder builder(absl::StrCat(dot.name(), "_fusion_space"));
  ...
  std::function<HloInstruction*(HloInstruction*)> FuseOperandsRecursively =
      [&](HloInstruction* instr) -> HloInstruction* {
    if (auto it = old_to_new.find(instr); it != old_to_new.end()) {
    ...
    old_to_new[instr] = param;
    return param;
  };

  // Starting from the root of the fusion, fuse upwards.
  HloInstruction* cloned_output = FuseOperandsRecursively(fusion_output);
...
absl::StatusOr<std::variant<Fusion, FusionDecision>> CreateDotFusionV2(
  ...
  return CreateFusionSpace(dot, gpu_version);
}
