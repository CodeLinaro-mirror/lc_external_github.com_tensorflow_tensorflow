...
bool CanUseTma(bool tma_enabled,
  ...
  SmallVector<uint64_t> global_shape_vec(tensor.getType().getShape().begin(),
                                         tensor.getType().getShape().end());
  SmallVector<uint32_t> tile_shape_vec(tile_shape.begin(), tile_shape.end());
  SmallVector<uint64_t> minor_to_major_layout_vec(minor_to_major_layout.begin(),
                                                  minor_to_major_layout.end());
  SmallVector<uint64_t> tile_strides_vec(canonical_tile_strides.begin(),
                                         canonical_tile_strides.end());
  auto tma_compatibilty_status = stream_executor::gpu::IsTmaCompatible(
      global_shape_vec, tile_shape_vec, tile_strides_vec,
      minor_to_major_layout_vec, element_byte_size);
