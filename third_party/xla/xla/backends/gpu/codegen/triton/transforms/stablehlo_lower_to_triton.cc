...
class LowerReduce : public mlir::OpRewritePattern<stablehlo::ReduceOp> {
 ...
 private:
  mlir::LogicalResult matchAndRewrite(
    ...
    if (IsNeutralInitValues(op, old_block)) {
      new_results = triton_reduce_op.getResults();
    } else {
  ...
  bool IsNeutralInitValues(stablehlo::ReduceOp op,
    ...
    if (std::distance(body_ops.begin(), body_ops.end()) != 1) {
      return false;
    }
    mlir::Operation& reducer_op = *body_ops.begin();

    for (Value init_val : op.getInitValues()) {
      ...
      if (elem_type.isF32() || elem_type.isF64() || elem_type.isF16() ||
        ...
        } else if (isa<arith::MulFOp, stablehlo::MulOp>(reducer_op)) {
          if (val != 1.0) {
            return false;
          }
        } else if (isa<arith::MaximumFOp, stablehlo::MaxOp>(reducer_op)) {
          if (!std::isinf(val) || val > 0) {
            return false;
          }
        } else if (isa<arith::MinimumFOp, stablehlo::MinOp>(reducer_op)) {
          if (!std::isinf(val) || val < 0) {
            return false;
          }
        } else {
      ...
      } else if (elem_type.isIntOrIndex()) {
        ...
        } else if (isa<arith::MulIOp, stablehlo::MulOp>(reducer_op)) {
          if (val != 1) {
            return false;
          }
        } else if (isa<arith::MaxSIOp>(reducer_op) ||
                   (isa<stablehlo::MaxOp>(reducer_op) && !is_unsigned)) {
          if (val != llvm::APInt::getSignedMinValue(bitwidth).getSExtValue()) {
            return false;
          }
        } else if (isa<arith::MaxUIOp>(reducer_op) ||
                   (isa<stablehlo::MaxOp>(reducer_op) && is_unsigned)) {
          if (val != 0) {
            return false;
          }
        } else if (isa<arith::MinSIOp>(reducer_op) ||
                   (isa<stablehlo::MinOp>(reducer_op) && !is_unsigned)) {
          if (val != llvm::APInt::getSignedMaxValue(bitwidth).getSExtValue()) {
            return false;
          }
        } else if (isa<arith::MinUIOp>(reducer_op) ||
                   (isa<stablehlo::MinOp>(reducer_op) && is_unsigned)) {
