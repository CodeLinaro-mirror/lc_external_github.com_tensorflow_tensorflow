...
LogicalResult UnswitchLoop(mlir::scf::ForOp for_op,
  ...
  // instances.
  scf::IfOp if_op;
  unsigned if_count = 0, max_if_count = 2;
  ...
  auto new_if = scf::IfOp::create(rewriter, for_op.getLoc(),
                                  for_op.getResultTypes(), if_op.getCondition(),
                                  /*addThenBlock=*/true, /*addElseBlock=*/true);

  for (auto body_index : {0, 1}) {
    ...
    auto new_for = builder.clone(*for_op, mapping);
