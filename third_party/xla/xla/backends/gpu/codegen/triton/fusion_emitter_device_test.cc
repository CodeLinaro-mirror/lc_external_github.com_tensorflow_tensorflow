...
%fused_add (param_0: pred[], param_1: pred[]) -> pred[] {
  %param_0 = pred[] parameter(0)
  %param_1 = pred[] parameter(1)
  ROOT %add = pred[] add(%param_0, %param_1)
}

ENTRY %main.1 (operands_0_.1: pred[2], operands_1_.1: pred[2]) -> pred[] {
  %operands_1_.1 = pred[] constant(1)
  %operands_0_.1 = pred[] constant(1)
  ROOT %add = pred[] fusion(%operands_0_.1, %operands_1_.1), kind=kCustom, calls=%fused_add,
...
%fused_reduce (p0: f32[2,2]) -> f32[2] {
  %p0 = f32[2,2]{1,0} parameter(0)
  %zero = f32[] constant(0)
  ROOT %reduce = f32[2]{0} reduce(%p0, %zero), dimensions={1}, to_apply=%unusual
}

ENTRY %entry_computation (p0: f32[2,2]) -> f32[2] {
  %p0 = f32[2,2]{1,0} parameter(0)
