...
class AllReduceEmitter {
 ...
 private:
  ...
  absl::Status Initialize() {
    ...
    const auto input_tile_shape_dims = ctx_.input_tile.getType().getShape();
    ...
    if (ctx_.strategy == AllReduceStrategy::kTwoShot) {
      ...
      // for n >= m.
      CHECK(Product(input_tile_shape.dimensions()) % Product(subtile_shape_) ==
            0)
  ...
  xtile::TensorValue CreateTensorOfRemoteBufferPtrs(
    ...
    const int32_t num_dims = tile_shape.size();
    ...
    llvm::SmallVector<int64_t> numsubtiles;
    numsubtiles.reserve(num_dims);
    for (auto [dim, subdim] : llvm::zip(tile_shape, subtile_shape)) {
      numsubtiles.push_back(dim / subdim);
    }
    const mlir::RankedTensorType tensor_of_numsubtile_ptrs =
        mlir::RankedTensorType::get(numsubtiles, ptr_to_elem_type_);
    // tensor<numsubtiles x i64>
    // NB: Product(numsubtiles) == world_size
    remote_buffers = ttir::ReshapeOp::create(
        builder_, tensor_of_numsubtile_ptrs, remote_buffers);
    // Expand the shape by a factor of 2 in each dimension.
    // So if original was 4x2, it becomes 4x1x2x1.
    // The broadcasted shape is numsubtiles x subtile_shape for each dimension.
    // 4, 1, 2, 1 -> 4, 32, 2, 64
    llvm::SmallVector<int64_t> expanded_shape(num_dims * 2);
    llvm::SmallVector<int64_t> broadcasted_shape(num_dims * 2);
    for (int i = 0; i < num_dims; ++i) {
      expanded_shape[2 * i] = numsubtiles[i];
      expanded_shape[2 * i + 1] = 1;  // Dim to be broadcasted (subtile)
      broadcasted_shape[2 * i] = numsubtiles[i];
      broadcasted_shape[2 * i + 1] = subtile_shape[i];
    }
    remote_buffers = ttir::ReshapeOp::create(
        builder_,
        mlir::RankedTensorType::get(expanded_shape, ptr_to_elem_type_),
        remote_buffers);
    // Broadcast to numsubtiles x subtile_shape
    remote_buffers = ttir::BroadcastOp::create(
        builder_,
        mlir::RankedTensorType::get(broadcasted_shape, ptr_to_elem_type_,
  ...
  absl::StatusOr<xtile::TensorValue> EmitGatherLoad() {
    ...
    // Each pointer within subtile i pointers to data in the remote buffer of
    // rank i.
  ...
  mlir::LogicalResult EmitTwoShot() {
    ...
    if (mlir::failed(EmitCopyToSymmetric(
            ctx_.input_tile, ctx_.input_extract.getOffsets(),
            ctx_.input_tile.getType().getShape()))) {
      LOG(ERROR) << "Failed to emit copy to symmetric for two-shot";
      return mlir::failure();
    }
    // 2. Shot1: Wait for all ranks to complete the copy.
    if (mlir::failed(EmitSync(signal_value_))) {
      LOG(ERROR) << "Failed to emit sync for shot1";
      return mlir::failure();
    }
    // 3. Reduce phase:
