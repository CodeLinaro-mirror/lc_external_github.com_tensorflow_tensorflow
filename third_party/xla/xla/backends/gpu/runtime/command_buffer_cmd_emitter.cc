...
class ConcurrentRegionScheduler {
 public:
  explicit ConcurrentRegionScheduler(const std::vector<Thunk*>& thunks)
      : thunks_(thunks) {
 ...
 private:
  ...
  void InitializeInDegrees() {
    for (size_t i = 0; i < thunks_.size(); ++i) {
      in_degree_[thunks_[i]] = 0;
    }
    for (auto& pair : successors_) {
      for (Thunk* successor : pair.second) {
        in_degree_[successor]++;
      }
    }
    if (VLOG_IS_ON(3)) {
      for (Thunk* thunk : thunks_) {
        VLOG(3) << "In-degree of " << thunk->thunk_info().profile_annotation
                << ": " << in_degree_[thunk];
      }
    }
  }
  ...
  void InitializeReady() {
    for (size_t i = 0; i < thunks_.size(); ++i) {
      if (in_degree_[thunks_[i]] == 0) {
        ready_[i % kMaxConcurrentOps].push_back(thunks_[i]);
        ready_size_++;
      }
    }
  }
  ...
  void Schedule() {
    ...
    while (ready_size() > 0) {
      ...
      for (size_t line_id = 0; line_id < active.size(); ++line_id) {
        Thunk* thunk = active.at(line_id);
        for (Thunk* successor : successors_[thunk]) {
          in_degree_.at(successor)--;
          if (in_degree_.at(successor) == 0) {
            EnqueueReady(successor, line_id);
          }
        }
        scheduled_thunks_.push_back(thunk);

        // Add dependencies to previously active ops.
        if (line_id < previously_active.size()) {
          Thunk* previously_active_same_lane = previously_active.at(line_id);
    ...
    // Record the last active thunks of this region.
    scheduled_sink_thunks_ = previously_active;
  }
  ...
  const std::vector<Thunk*>& thunks_;
  absl::flat_hash_map<const Thunk*, std::set<Thunk*>> successors_;
  absl::flat_hash_map<const Thunk*, int64_t> in_degree_;
...
absl::Status AppendCommandsInConcurrentRegions(
  ...
  for (int64_t concurrent_region_id : concurrent_region_ids) {
    ...
    ConcurrentRegionScheduler scheduler(thunks);
    concurrent_region_schedules.emplace_back(scheduler);
  ...
  for (size_t i = 1; i < concurrent_region_schedules.size(); ++i) {
    for (Thunk* sink :
      ...
      for (Thunk* source :
        ...
        ctx.extra_resources[thunk_to_index[sink]].push_back(
            ResourceUse::Read(cmd_sequence[thunk_to_index[source]]->token()));
      };
    }
  }
  return absl::OkStatus();
