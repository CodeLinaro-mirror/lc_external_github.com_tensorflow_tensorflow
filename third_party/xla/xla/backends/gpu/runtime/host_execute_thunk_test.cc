...
TEST(HostExecuteStartThunkTest, BufferUses) {
  static constexpr char const* kHloModule = R"hlo(
    HloModule module
    ENTRY add_inplace {
      p0 = s32[] parameter(0)
      ROOT add = s32[] add(p0, p0)
  ...
  BufferAllocation alloc_arg(/*index=*/0, /*size=*/4, /*color=*/0);
  BufferAllocation alloc_result(/*index=*/1, /*size=*/4, /*color=*/0);

  BufferAllocation::Slice slice_arg(&alloc_arg, /*offset=*/0, /*size=*/4);
  BufferAllocation::Slice slice_result(&alloc_result, /*offset=*/0, /*size=*/4);

  Shape shape = ShapeUtil::MakeShape(S32, {});

  ASSERT_OK_AND_ASSIGN(
      auto thunk, CreateHostExecuteStartThunk(Thunk::ThunkInfo(), *hlo_module,
                                              {{slice_arg, shape}},
                                              {{slice_result, shape}}));

  EXPECT_THAT(thunk->buffer_uses(),
              ElementsAre(BufferUse::Read(slice_arg, shape),
                          BufferUse::Write(slice_result, shape)));
}
