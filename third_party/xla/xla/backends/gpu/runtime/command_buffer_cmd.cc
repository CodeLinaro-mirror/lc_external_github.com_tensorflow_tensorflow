...
static void VlogOperations(const std::vector<CommandOperation>& operations) {
  if (VLOG_IS_ON(2)) {
...
static std::vector<CommandOperation> CreateCommandOperationsWithConcurrentMode(
...
int64_t FindMatchingStartId(const std::vector<CommandOperation>& ops,
  ...
  for (int64_t j = done_idx - 1; j >= 0; --j) {
    ...
    const auto* start_cmd = dynamic_cast<const CollectiveCmd*>(ops[j].cmd());
    CHECK(start_cmd);
...
void AddDependencyOnPrevNonStart(std::vector<CommandOperation>& ops,
  ...
  for (int64_t j = current_idx - 1; j >= 0; --j) {
    ...
    ops[current_idx].add_resouce_use(ResourceUse::Read(commands[j]->token()));
...
static std::vector<CommandOperation> CreateCommandOperationsWithLHSMode(
    const CommandSequence& commands) {
...
static absl::StatusOr<std::vector<CommandOperation>> CreateCommandOperations(
  ...
  using Mode = CommandBufferCmdExecutor::SynchronizationMode;
