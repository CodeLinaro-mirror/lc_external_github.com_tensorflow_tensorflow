...
const OrderedEstimatesAndConfigs* GetOrEstimateConfigs(
    const EstimationContext& context,
    const std::vector<TritonGemmConfig>& configs,
    mlir::MLIRContext* mlir_context,
    std::optional<absl::StatusOr<OrderedEstimatesAndConfigs>>& cache) {
  if (!cache.has_value()) {
    cache = EstimateConfigs(context, configs, mlir_context);
  }
  if (!cache->ok()) {
    VLOG(1) << "Cost Model: Failed to estimate configs: " << cache->status();
    return nullptr;
  }
  return &cache->value();
}
...
CostModelGemmTilingOptions ParseCostModelGemmTilingOptions(
  ...
  for (const auto& [key, value] : tsl::SortedRange(options)) {
    if (key == "top") {
      ...
      if (absl::SimpleAtoi(value, &val)) {
        ...
        LOG(WARNING) << "Could not parse 'top' value: " << value;
      }
      ...
      if (absl::SimpleAtoi(value, &val)) {
        ...
        LOG(WARNING) << "Could not parse 'top_from_default' value: " << value;
      }
    } else if (key == "mixin") {
      int val = 0;
      if (absl::SimpleAtoi(value, &val)) {
        ...
        LOG(WARNING) << "Could not parse 'mixin' value: " << value;
      }
    } else if (key == "filter") {
      float val = 0.f;
      if (absl::SimpleAtof(value, &val)) {
        ...
        LOG(WARNING) << "Could not parse 'filter' value: " << value;
      }
    }
  }
...
std::vector<TritonGemmConfig> OptimizeConfigsWithCostModel(
  ...
  // Lazily evaluated estimation - if it fails, returns nullptr.
  auto get_estimated_all_configs = [&]() {
    return detail::GetOrEstimateConfigs(context, all_configs, mlir_context,
                                        estimated_all_configs);
  };

  std::optional<absl::StatusOr<detail::OrderedEstimatesAndConfigs>>
      estimated_optimized_configs;
  // Lazily evaluated estimation - if it fails, returns nullptr.
  auto get_estimated_optimized_configs = [&]() {
    return detail::GetOrEstimateConfigs(
        context, optimized_configs, mlir_context, estimated_optimized_configs);
  };
  ...
  if (options.top.has_value()) {
    bool top_from_default = options.top_from_default.value_or(false);

    const detail::OrderedEstimatesAndConfigs* base_config_set =
        top_from_default ? get_estimated_optimized_configs()
    ...
    VLOG(1) << "Cost Model: Selecting top " << *options.top << " configs from "
            << (top_from_default ? "default" : "exhaustive") << " set";
    ...
    const detail::OrderedEstimatesAndConfigs* base_config_set =
        get_estimated_optimized_configs();
    if (!base_config_set) {
      return optimized_configs;
    }
    current_set = *base_config_set;
  }
  ...
  if (options.mixin.has_value()) {
    ...
    const detail::OrderedEstimatesAndConfigs* all = get_estimated_all_configs();
    if (!all) {
      return optimized_configs;
    }
