...
class InputSpec:
  ...
  def __init__(self, shape: tuple[int, ...], use_arrange: bool = False):
    """Initializes the InputSpec.

    Args:
      shape: The shape of the input array.
      use_arrange: If true uses np.arrange to create the input array, otherwise
        generates a random array.
    """
    self.shape = shape
    self.use_arrange = use_arrange
...
class XtileLoweringTest(absltest.TestCase):
  ...
  def test_dot_single_tile(self):
    ...
    compare_kernel(
        ...
        "dot_single_tile",
        1,
        [InputSpec((8, 16), True), InputSpec((16, 8), True)],
        (8, 8),
        np.float32,
  ...
  def test_reduction_add_inner(self):
    ...
    compare_kernel(
        ...
        "reduction_add_inner",
        4,
        [InputSpec((1024, 32), True), InputSpec((1,), True)],
        (1024,),
        np.int32,
  ...
  def test_reduction_middle(self):
    ...
    compare_kernel(
        ...
        1,
        [InputSpec((8, 4, 2), True), InputSpec((1,), True)],
        (8, 2),
        np.float32,
