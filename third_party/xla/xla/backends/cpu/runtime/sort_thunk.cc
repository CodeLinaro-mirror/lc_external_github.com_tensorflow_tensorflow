...
tsl::AsyncValueRef<SortThunk::ExecuteEvent> SortThunk::Execute(
  ...
  // If there is only one slice or no threadpool, sort sequentially on the
  // caller thread.
  if (params.intra_op_threadpool == nullptr || num_slices == 1) {
  ...
  sort_slice_range(0, std::min(num_slices, grain_slices));
  event.CountDown();
