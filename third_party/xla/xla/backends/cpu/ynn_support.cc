...
bool IsConvolutionOpSupportedByYnn(const HloInstruction* instr) {
  ...
  bool all_strides_are_one =
      window.dimensions(0).stride() == 1 && window.dimensions(1).stride() == 1;
  // Currently only support grouped convolutions when all strides are 1, because
  // of the issue with split_dim using reshape.
  if (conv->feature_group_count() > 1 && !all_strides_are_one) {
    return false;
  }
