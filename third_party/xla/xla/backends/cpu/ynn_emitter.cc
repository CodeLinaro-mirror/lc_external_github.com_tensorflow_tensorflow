...
static ynn_status DefineConvolution(
    ...
    const std::vector<size_t>& out_dims, size_t feature_group_count,
    size_t input_channels, size_t kernel_output_channels,
  ...
  if (feature_group_count != 1) {
    ...
    status = ynn_define_split_dim(subgraph, 3, 3, input_split, input1_id,
                                  &split_id, /*flags=*/0);
    if (status != ynn_status_success) {
      return status;
    }
    ...
    status = ynn_define_split_dim(subgraph, 3, 2, filter_split, input2_id,
                                  &split_id, /*flags=*/0);
    if (status != ynn_status_success) {
      return status;
    }
    ...
    status = ynn_define_static_transpose(subgraph, 5, swap_co_ci, input2_id,
    ...
    std::vector<size_t> unfused_dims(out_dims.begin(), out_dims.begin() + 3);
  ...
  if (feature_group_count > 1) {
    ...
    status = ynn_define_fuse_dim(subgraph, 3, 3, output_unfused_id, &output_id,
                                 /*flags=*/0);
    if (status != ynn_status_success) {
      return status;
    }
...
static absl::StatusOr<YnnSubgraph> EmitYnnConvolutionSubgraph(
  ...
  YNN_RETURN_IF_ERROR(
      DefineConvolution(subgraph.get(), ynn_lhs_type, ynn_out_type, lhs_id,
                        rhs_id, out_id, out_dims, conv->feature_group_count(),
