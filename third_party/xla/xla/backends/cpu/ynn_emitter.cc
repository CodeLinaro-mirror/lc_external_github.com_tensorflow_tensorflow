...
static ynn_status DefineConvolution(
  ...
  std::vector<size_t> grouped_stencil_dims = stencil_dims;
  std::vector<int32_t> grouped_stencil_axes = stencil_axes;
  std::vector<int32_t> grouped_new_axes(stencil_axes.size());
  std::vector<size_t> grouped_stencil_strides = stencil_strides;
  std::vector<size_t> grouped_stencil_dilations = stencil_dilations;

  if (feature_group_count != 1) {
    // (n, y, x, c) -> (n, y, x, [g, 1,] dy, dx, c)
    grouped_stencil_dims.push_back(feature_group_count);
    grouped_stencil_dims.push_back(1);
    grouped_stencil_axes.push_back(3);
    grouped_stencil_axes.push_back(3);
    grouped_new_axes = {-3, -2, -5, -4};
    grouped_stencil_strides.push_back(1);
    grouped_stencil_strides.push_back(1);
    grouped_stencil_dilations.push_back(input_channels / feature_group_count);
    grouped_stencil_dilations.push_back(1);
  } else {
    grouped_new_axes = {-3, -2};
  }
  ...
  status = ynn_define_stencil_copy(
      subgraph, /*num_stencils=*/grouped_stencil_dims.size(),
      grouped_stencil_axes.data(), grouped_new_axes.data(),
      grouped_stencil_dims.data(), grouped_stencil_strides.data(),
      grouped_stencil_dilations.data(), input1_id, YNN_INVALID_VALUE_ID,
      &stencil_id,
      /*flags=*/0);
  if (status != ynn_status_success) {
    return status;
...
static absl::StatusOr<YnnSubgraph> EmitYnnConvolutionSubgraph(
  ...
  YNN_RETURN_IF_ERROR(DefineConvolution(
      ...
      stencil_axes, stencil_dims, stencil_strides, stencil_dilations,
