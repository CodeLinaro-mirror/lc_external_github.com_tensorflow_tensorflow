 ...
 *   - All users of the quantize op must be either quant.dequantize composites.
 *
 * - **quant.dequantize:**
 *   - Requires exactly one operand and one result.
 *   - Legalization is supported only if the operand is:
 *     - quant.quantize composite op or quantized type.
...
bool isQuantizeCompositeOp(stablehlo::CompositeOp op) {
...
bool isDequantizeCompositeOp(stablehlo::CompositeOp op) {
...
bool isFakeQuantCompositeOp(stablehlo::CompositeOp op) {
...
FailureOr<stablehlo::UniformQuantizeOp> buildUniformQuantizeOp(
  ...
  return rewriter.create<stablehlo::UniformQuantizeOp>(
...
class RewriteDequantizeCompositeOp
  ...
  LogicalResult matchAndRewrite(stablehlo::CompositeOp op,
    ...
    if (!quantizeCompositeOp || !isQuantizeCompositeOp(quantizeCompositeOp)) {
      return rewriter.notifyMatchFailure(
