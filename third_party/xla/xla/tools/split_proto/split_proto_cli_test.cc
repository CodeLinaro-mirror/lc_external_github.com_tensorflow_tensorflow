...
TEST(SplitProtoCliTest, PackAndUnpackGpuExecutableProtoRoundTrip) {
  ...
  EXPECT_EQ(final_proto.asm_text(), initial_proto.asm_text());
  EXPECT_EQ(final_proto.binary(), initial_proto.binary());
  ASSERT_EQ(final_proto.constants_size(), 1);
  EXPECT_EQ(final_proto.constants(0).symbol_name(), "const_1");
  EXPECT_EQ(final_proto.constants(0).content().data(), "const_1_data");
}
...
TEST(SplitProtoCliTest, PackAndUnpackExecutableAndOptionsProtoRoundTrip) {
  ...
  EXPECT_EQ(
      final_proto.compile_options().executable_build_options().device_ordinal(),
      2);
  ASSERT_EQ(final_proto.compile_options()
                .executable_build_options()
                .result_layout()
                .dimensions_size(),
            3);
  EXPECT_EQ(final_proto.compile_options()
                .executable_build_options()
                .result_layout()
                .dimensions(0),
            1);
...
TEST(SplitProtoCliTest, PackInvalidProtoType) {
  ...
  EXPECT_FALSE(Pack(std::move(reader), std::move(writer), pack_opts).ok());
}
