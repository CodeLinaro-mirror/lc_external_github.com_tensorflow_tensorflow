...
int main(int argc, char** argv) {
  // When running via bazel run, the working directory is changed to the
  // runfiles directory. Reset it to the directory that the user invoked bazel
  // run from to allow finding relative files.
  if (const char* working_dir = std::getenv("BUILD_WORKING_DIRECTORY")) {
    if (chdir(working_dir) != 0) {
      std::cerr
          << "Failed to change working directory to BUILD_WORKING_DIRECTORY: "
          << working_dir << "\n";
      return 1;
    }
  }
