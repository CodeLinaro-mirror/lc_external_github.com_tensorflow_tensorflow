...
absl::StatusOr<xla::ifrt::DeviceListRef> MakeDeviceListFromAtomDeviceIds(
    ...
    const std::vector<int32_t>& atom_device_ids) {
  ...
  for (const auto& atom_device_id : atom_device_ids) {
    TF_ASSIGN_OR_RETURN(
        ...
        client.LookupDevice(device_assignments[atom_device_id]));
...
absl::Status IfrtIrExecutableVersion::IsCompatibleWith(
  ...
  for (const auto& [other_atom_abi_version, other_atom_devices] :
       other_ifrt_ir_executable_version->runtime_abi_versions) {
    absl::StatusOr<xla::ifrt::DeviceListRef> other_atom_device_list =
        MakeDeviceListFromAtomDeviceIds(
            client, other_ifrt_ir_executable_version->device_assignments,
            other_atom_devices);
    if (!other_atom_device_list.ok()) {
      return other_atom_device_list.status();
    }
    TF_RETURN_IF_ERROR(
...
absl::StatusOr<IfrtIrExecutableVersionProto> IfrtIrExecutableVersion::ToProto(
  ...
  for (const auto& [runtime_abi_version, atom_logical_device_ids] :
    ...
    atom_executable_version_proto.mutable_logical_device_indexes()->Add(
        atom_logical_device_ids.begin(), atom_logical_device_ids.end());
...
IfrtIrExecutableVersion::FromProto(
  ...
  for (const auto& atom_executable_version_proto :
    ...
    for (auto logical_device_id :
      ...
      atom_executable_version.devices.push_back(logical_device_id);
    }
    ...
    if (auto xla_executable_version =
      ...
      runtime_abi_version_strs.push_back(absl::StrCat(
          ...
          absl::StrJoin(atom_devices, ",",
                        [](std::string* out, uint32_t device_id) {
                          absl::StrAppend(out, device_id);
                        }),
          "]}"));
    } else {
