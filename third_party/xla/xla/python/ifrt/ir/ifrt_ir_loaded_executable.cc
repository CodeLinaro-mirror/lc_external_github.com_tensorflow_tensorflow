...
absl::StatusOr<absl::flat_hash_map<xla::ifrt::DeviceId, int32_t>>
CreateDeviceIdToLogicalDeviceIdMap(
    std::shared_ptr<CompiledIfrtIrProgram> program) {
  absl::flat_hash_map<xla::ifrt::DeviceId, int32_t>
...
IfrtIrLoadedExecutable::executable_version() const {
  absl::call_once(version_once_, [&]() {
    version_ = [&]()
      ...
      TF_ASSIGN_OR_RETURN((absl::flat_hash_map<xla::ifrt::DeviceId, int32_t>
                               device_id_to_logical_device_id),
