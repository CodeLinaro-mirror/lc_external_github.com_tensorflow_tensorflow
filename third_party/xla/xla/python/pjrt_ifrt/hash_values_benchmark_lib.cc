...
void BM_HashValues(benchmark::State& state, Client::HashMode hash_mode,
  ...
  if (client == nullptr) {
    LOG(ERROR) << "Failed to obtain IFRT client";
    return;
  }
  ...
  if (static_cast<int64_t>(available_devices.size()) < num_devices) {
    LOG(ERROR) << "Not enough devices available for benchmark. Requested: "
  ...
  for (int64_t i = 0; i < num_arrays; ++i) {
    ...
    if (!array.ok()) {
      LOG(ERROR) << "Failed to create sharded array: " << array.status();
      return;
    }
  ...
  for (auto _ : state) {
    ...
    if (!hash.ok()) {
      LOG(ERROR) << "HashValues failed: " << hash.status();
      break;
    }
  }
...
void CustomBenchmarkArgs(benchmark::Benchmark* b) {
...
void BM_HashValues_Physical_Device_bench(benchmark::State& state) {
  BM_HashValues(state, Client::HashMode::kPhysical, kDeviceMemoryKind);
}

void BM_HashValues_Physical_PinnedHost_bench(benchmark::State& state) {
  BM_HashValues(state, Client::HashMode::kPhysical, kPinnedHostMemoryKind);
}

void BM_HashValues_Logical_Device_bench(benchmark::State& state) {
  BM_HashValues(state, Client::HashMode::kLogical, kDeviceMemoryKind);
}

void BM_HashValues_Logical_PinnedHost_bench(benchmark::State& state) {
  BM_HashValues(state, Client::HashMode::kLogical, kPinnedHostMemoryKind);
}

BENCHMARK(BM_HashValues_Physical_Device_bench)
    ->UseRealTime()
    ->Apply(CustomBenchmarkArgs);
BENCHMARK(BM_HashValues_Physical_PinnedHost_bench)
    ->UseRealTime()
    ->Apply(CustomBenchmarkArgs);
BENCHMARK(BM_HashValues_Logical_Device_bench)
    ->UseRealTime()
    ->Apply(CustomBenchmarkArgs);
BENCHMARK(BM_HashValues_Logical_PinnedHost_bench)
    ->UseRealTime()
    ->Apply(CustomBenchmarkArgs);
